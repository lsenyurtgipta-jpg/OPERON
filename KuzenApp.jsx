...
const SATICI_UI={
  ...
  // Tanıtım slider (ADIM 2): cap KALDIRILDI. Kuti parent'ın kalan alanını tam doldurur (height/width:100%),
  // görsel object-fit:contain ile sınırlanır. iPad/desktop fark etmez — magic number yok.
  ...
  // ── Satıcı FİRMA KARTI (iPad, sade) ──
  kartMaxW:980,           // firma kartı max genişlik (iPad'de ortalı; dar ekranda %100)
  kartInpH:44,            // input/dokunma yüksekliği (Apple HIG min 44px)
  kartGap:14,             // bölüm kartları arası boşluk
  kartMinCol:220,         // alan grid'i min kolon (auto-fit → parent-fit, sabit kolon yok)
};
...
const isSatici = (user) => user && user.rol === "satici";

/* Yetki matrisi — sayfa erişimleri.
   Yetkisiz kullanıcı sayfaya gelirse useEffect ile dashboard'a düşer. */
const PAGE_ROLES = {
  dashboard:    ['admin','data_entry'],
  firmalar:     ['admin','data_entry','satici'],
  malzemeler:   ['admin','data_entry'],
  projeler:     ['admin','data_entry'],
  teklifler:    ['admin','data_entry'],
  satinalma:    ['admin','data_entry'],
  teklif_verme: ['admin','data_entry'],
  satinalma_sp: ['admin','data_entry'],
  alis_fatura:  ['admin','data_entry'],
  satis_fatura: ['admin','data_entry'],
...
const HOME_PAGE_BY_ROL = {
  admin:      "dashboard",
  data_entry: "dashboard",
  satici:     "satis_sunum",
};

const canSee = (pageId, user) => {
  if(!user) return false;
  const allowed = PAGE_ROLES[pageId];
  return !allowed || allowed.includes(user.rol);
};

const rolLabel = (rol) => ({admin:"Yönetici", data_entry:"Veri Girişi", satici:"Satıcı"}[rol] || rol);
...
const KullanicilarPage = ({currentUser}) => {
  ...
  const [yeni, setYeni] = useState({ kullanici_adi:"", sifre:"", rol:"data_entry", ad_soyad:"", telefon:"", eposta:"" });
  ...
  const yeniKaydet = async () => {
    ...
    try {
      ...
      await sbPost('kullanicilar', {
        kullanici_adi:k, sifre_hash:hash, rol:yeni.rol,
        sifre_degistirildi:false, aktif:true,
        ad_soyad:(yeni.ad_soyad||"").trim(),
        telefon:(yeni.telefon||"").trim(),
        eposta:(yeni.eposta||"").trim(),
      });
      setYeni({ kullanici_adi:"", sifre:"", rol:"data_entry", ad_soyad:"", telefon:"", eposta:"" });
      setYeniOpen(false);
      await load();
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>
          {users.map(u => <tr key={u.id} style={{borderBottom:`1px solid ${T.border}`}}>
            ...
            <td style={{padding:"12px 16px",fontSize:"13px",color:T.t2}}>{rolLabel(u.rol)}</td>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",padding:"32px",borderRadius:"12px",width:"420px",display:"flex",flexDirection:"column",gap:"14px"}}>
        ...
        <div>
          ...
          <select value={yeni.rol} onChange={e=>setYeni({...yeni,rol:e.target.value})} disabled={yeniBusy}
            ...
            <option value="satici">Satıcı (saha satışı)</option>
          </select>
        </div>
        {yeni.rol === "satici" && <>
          <div>
            <label style={{display:"block",fontSize:"13px",color:T.t2,marginBottom:"6px",fontWeight:500}}>Ad Soyad</label>
            <input value={yeni.ad_soyad} onChange={e=>setYeni({...yeni,ad_soyad:e.target.value})} disabled={yeniBusy} placeholder="örn. Ahmet Yılmaz"
              style={{width:"100%",height:"38px",padding:"0 12px",borderRadius:T.r,border:`1px solid ${T.bDark}`,fontSize:"14px",boxSizing:"border-box"}}/>
          </div>
          <div>
            <label style={{display:"block",fontSize:"13px",color:T.t2,marginBottom:"6px",fontWeight:500}}>Telefon</label>
            <input value={yeni.telefon} onChange={e=>setYeni({...yeni,telefon:e.target.value})} disabled={yeniBusy} placeholder="örn. 0532 123 45 67"
              style={{width:"100%",height:"38px",padding:"0 12px",borderRadius:T.r,border:`1px solid ${T.bDark}`,fontSize:"14px",boxSizing:"border-box"}}/>
          </div>
          <div>
            <label style={{display:"block",fontSize:"13px",color:T.t2,marginBottom:"6px",fontWeight:500}}>E-posta</label>
            <input type="email" value={yeni.eposta} onChange={e=>setYeni({...yeni,eposta:e.target.value})} disabled={yeniBusy} placeholder="ornek@firma.com"
              style={{width:"100%",height:"38px",padding:"0 12px",borderRadius:T.r,border:`1px solid ${T.bDark}`,fontSize:"14px",boxSizing:"border-box"}}/>
          </div>
        </>}
...
const Sidebar=({page,setPage,open,editMode=false,currentUser=null})=>{
  ...
  const items=allItems.map(it=>{
    if(it.type==="group"){
      const fc=it.children.filter(c=>canSee(c.id,currentUser));
      return fc.length>0 ? {...it,children:fc} : null;
    }
    return canSee(it.id,currentUser) ? it : null;
  }).filter(Boolean);
  ...
  return <div style={{width:open?"264px":"0",minWidth:open?"264px":"0",height:"100%",background:SB.bg,display:"flex",flexDirection:"column",transition:"width .3s cubic-bezier(.2,0,0,1),min-width .3s cubic-bezier(.2,0,0,1)",overflow:"hidden"}}>
    ...
    <div style={{padding:"16px 20px",borderTop:"1px solid rgba(255,255,255,0.08)",display:"flex",alignItems:"center",gap:"10px"}}>
      <div style={{width:"32px",height:"32px",borderRadius:"50%",background:"#8799a3",display:"flex",alignItems:"center",justifyContent:"center",color:"#384248",fontWeight:600,fontSize:"13px",flexShrink:0}}>{(currentUser?.kullaniciAdi||"?").charAt(0).toUpperCase()}</div>
      <div><div style={{color:"rgba(255,255,255,0.85)",fontSize:"13px",fontWeight:500,whiteSpace:"nowrap",fontFamily:SB.font}}>{currentUser?.kullaniciAdi||"-"}</div><div style={{color:"rgba(255,255,255,0.4)",fontSize:"11px",whiteSpace:"nowrap",fontFamily:SB.font}}>{rolLabel(currentUser?.rol)}</div></div>
    </div>
            ...
            {opsiyonluDaireler.length===0
              ...
              :<div style={{border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden"}}>
                {opsiyonluDaireler.map((d,idx)=><div key={`${d.projeId}_${d.id}`} onClick={()=>setPage&&setPage("satis_sunum")} style={{display:"grid",gridTemplateColumns:"1fr 80px 80px 110px 120px",gap:"12px",padding:"10px 14px",background:idx%2===0?"#fffbe6":"#fff",borderBottom:idx<opsiyonluDaireler.length-1?`1px solid ${T.border}`:"none",cursor:"pointer",alignItems:"center"}}
                  ...
                  <div style={{fontSize:"13px",fontWeight:600,color:"#fa8c16",textAlign:"right"}} title="Satış Bedeli + PLUS (KDV Hariç)">{(()=>{const n=(parseFloat(d.satisBedeli)||0)+(parseFloat(d.plus)||0);return n>0?n.toLocaleString("tr-TR")+" ₺":"—";})()}</div>
                </div>)}
              </div>}
          </div>
            ...
            {satinAlinanDaireler.length===0
              ...
              :<div style={{border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden"}}>
                {satinAlinanDaireler.map((d,idx)=><div key={`${d.projeId}_${d.id}`} onClick={()=>setPage&&setPage("satis_sunum")} style={{display:"grid",gridTemplateColumns:"1fr 80px 80px 110px 120px",gap:"12px",padding:"10px 14px",background:idx%2===0?"#e6f4ff":"#fff",borderBottom:idx<satinAlinanDaireler.length-1?`1px solid ${T.border}`:"none",cursor:"pointer",alignItems:"center"}}
                  ...
                  <div style={{fontSize:"12px",color:T.t2}}>Daire {d.no||"—"}</div>
                  <div style={{fontSize:"12px",color:T.t2}}>{d.brutM2?`${d.brutM2} m²`:"—"} {d.odaSayisi?`• ${d.odaSayisi}`:""}</div>
                  <div style={{fontSize:"13px",fontWeight:700,color:"#1677ff",textAlign:"right"}} title="Satış Bedeli + PLUS (KDV Hariç)">{(()=>{const n=(parseFloat(d.satisBedeli)||0)+(parseFloat(d.plus)||0);return n>0?n.toLocaleString("tr-TR")+" ₺":"—";})()}</div>
                </div>)}
              </div>}
          </div>
        </div>}
...
/* ========== SATICI FİRMA KARTI (iPad — sadeleştirilmiş) ==========
   Admin FirmaKarti'ye DOKUNMADAN, sales rolü için tek-kolon parent-fit kart.
   Ölçüler SATICI_UI.kart* config'inden okunur (magic number dağıtılmaz).
   Görünür: müşteri/iletişim/adres/yakınları/satış-sunum/notlar.
   Katlanır: Resmi-Vergi (tüzel↔şahıs koşullu; alıcıya dönüştürmek için gerekli).
   Gizli: banka, şube, çoklu adres, web, sicil no. */
const SaticiFirmaKarti=({firma,initData,isNew,onSave,onBack,onAddNote,firmalar,projeler=[],setPage,sunumlar=[]})=>{
  const[form,setForm]=useState(()=>firma?{...firma}:{
    id:Date.now(),firmaKodu:initData?.firmaKodu||"",ad:"",kisaAd:"",aciklama:"",turler:initData?.turler||["potansiyel"],paraBirimi:initData?.paraBirimi||"TL",
    firmaKisiTipi:"sahis",
    vergiDairesiIl:"",vergiDairesi:"",vergiNo:"",tcKimlikNo:"",sicilNo:"",kategori:"",musteriTipi:"",
    telefon:"",sabitTelefon:"",telefon2:"",webAdresi:"",eposta:"",
    mahalle:"",adres:"",il:"",ilce:"",postaKodu:"",
    bankaAdi:"",iban:"",aktif:true,
    ilgilendigiProjeler:[],kisiler:[],notlar:[],belgeler:[],subeler:[],bankalar:[],iletisimler:[],adresler:[],
    createdAt:new Date().toISOString().split("T")[0]
  });
  const[saved,setSaved]=useState(false);
  ...
  // Yakınlar (kisiler[]) — "yakınlık derecesi" mevcut `unvan` alanına yazılır (Supabase değişmez)
  const addYakin=()=>setForm(p=>({...p,kisiler:[...(p.kisiler||[]),{id:Date.now(),cinsiyet:"",ad:"",soyad:"",unvan:"",departman:"",telefon:"",isTel:"",isTelDahili:"",cep:"",eposta:"",sosyal1:"",sosyal2:"",sosyal3:"",dogumTarihi:"",notlar:"",resim:""}]}));
  const upYakin=(i,f,v)=>setForm(p=>{const k=[...(p.kisiler||[])];k[i]={...k[i],[f]:v};return{...p,kisiler:k};});
  const rmYakin=(i)=>setForm(p=>({...p,kisiler:(p.kisiler||[]).filter((_,j)=>j!==i)}));

  const addN=()=>{if(!nn.trim())return;if(onAddNote&&firma){onAddNote(firma.id,nn);}else{const n={id:Date.now(),tarih:new Date().toISOString().split("T")[0],yazar:"Satıcı",metin:nn};setForm(p=>({...p,notlar:[...(p.notlar||[]),n]}));}setNn("");};

  const save=async()=>{
    if(savingRef.current)return; // çift dokunuş / kayıt sürerken ikinci tetik → mükerrer engeli
    if(!form.ad.trim()){alert("Müşteri adı zorunludur!");return;}
    if(!phoneOk(form.telefon)){alert("Cep telefonu hatalı! (boş veya 11 hane olmalı)");return;}
    if(!phoneOk(form.sabitTelefon)){alert("Sabit telefon hatalı! (boş veya 11 hane olmalı)");return;}
    const badK=(form.kisiler||[]).find(k=>k.telefon&&!phoneOk(k.telefon));
    if(badK){alert(`Yakın "${badK.ad||"?"} ${badK.soyad||""}" telefonu hatalı!`);return;}
  ...
  const isAlici=(form.turler||[]).includes("alici");
  const sadecePotansiyel=(form.turler||[]).includes("potansiyel")&&!isAlici;
  const aliciValidasyon=()=>{
    const eksik=[];
    if(!form.ad.trim())eksik.push(form.firmaKisiTipi==="sahis"?"Ad Soyad":"Ünvan");
    if(!form.telefon||!phoneOk(form.telefon))eksik.push("Telefon (11 hane)");
    if(!form.adres||!form.adres.trim())eksik.push("Adres");
    if(!form.il||!form.il.trim())eksik.push("İl");
    if(form.firmaKisiTipi==="sahis"){if(!form.tcKimlikNo||!/^\d{11}$/.test(form.tcKimlikNo))eksik.push("TC Kimlik No (11 hane)");}
    else{if(!form.vergiNo||!/^\d{10}$/.test(form.vergiNo))eksik.push("Vergi No (10 hane)");if(!form.vergiDairesi||!form.vergiDairesi.trim())eksik.push("Vergi Dairesi");}
    return eksik;
  };
  const aliciyaDonustur=()=>{
    const eksik=aliciValidasyon();
  ...
  const opsiyonluDaireler=useMemo(()=>{if(!isAlici||!form.id)return[];const out=[];projeler.forEach(p=>{(p.bolumler||[]).forEach(b=>{if(b.aliciFirmaId===form.id&&b.durum==="opsiyonlu")out.push({...b,projeId:p.id,projeAd:p.ad});});});return out;},[projeler,form.id,isAlici]);
  const satinAlinanDaireler=useMemo(()=>{if(!isAlici||!form.id)return[];const out=[];projeler.forEach(p=>{(p.bolumler||[]).forEach(b=>{if(b.aliciFirmaId===form.id&&b.durum==="satildi")out.push({...b,projeId:p.id,projeAd:p.ad});});});return out;},[projeler,form.id,isAlici]);
  const firmaSunumlari=useMemo(()=>(sunumlar||[]).filter(s=>String(s.firmaId)===String(form.id)).sort((a,b)=>String(b.tarih||"").localeCompare(String(a.tarih||""))),[sunumlar,form.id]);
  const projeAdOf=(pid)=>(projeler.find(p=>String(p.id)===String(pid))?.ad)||"";
  const daireAdOf=(pid,bid)=>{const p=projeler.find(p=>String(p.id)===String(pid));const b=(p?.bolumler||[]).find(bb=>String(bb.id)===String(bid));return b?`${b.blok?b.blok+" Blok • ":""}Daire ${b.no||"?"}`:"";};
  const bedelOf=(d)=>{const n=(parseFloat(d.satisBedeli)||0)+(parseFloat(d.plus)||0);return n>0?n.toLocaleString("tr-TR")+" ₺":"—";};
  const sonucEt={ilgileniyor:{l:"İlgileniyor",c:"#52c41a",bg:"#f6ffed"},dusunuyor:{l:"Düşünüyor",c:"#fa8c16",bg:"#fff7e6"},pas_gecti:{l:"Pas geçti",c:"#8c8c8c",bg:"#f5f5f5"}};
  const ilgiEt={yuksek:{l:"Yüksek ilgi",c:"#ff4d4f",bg:"#fff1f0"},orta:{l:"Orta ilgi",c:"#fa8c16",bg:"#fff7e6"},dusuk:{l:"Düşük ilgi",c:"#8c8c8c",bg:"#f5f5f5"}};

  // stiller (ölçüler config'ten — magic number dağıtma)
  const inp={...iS,height:SATICI_UI.kartInpH+"px",lineHeight:SATICI_UI.kartInpH+"px"};
  const fl={display:"block",fontSize:"13px",fontWeight:600,color:T.t2,marginBottom:"6px"};
  ...
  return <div style={{maxWidth:SATICI_UI.kartMaxW+"px",width:"100%",margin:"0 auto",boxSizing:"border-box",display:"flex",flexDirection:"column",gap:SATICI_UI.kartGap+"px"}}>
    ...
    {/* HEADER */}
    <div style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px 16px",background:T.sidebar,borderRadius:T.rl}}>
      <button onClick={onBack} title="Geri" style={{padding:0,border:"none",background:"transparent",color:"#8799a3",cursor:"pointer",display:"flex",alignItems:"center",flexShrink:0}}><MoveLeft size={30}/></button>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:"18px",fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{form.ad||"Yeni Müşteri"}</div>
        {form.firmaKodu&&<div style={{fontSize:"12px",color:"#8799a3",fontFamily:"monospace"}}>{form.firmaKodu}</div>}
      </div>
      <div style={{flexShrink:0}}><AktifPasifToggle aktif={form.aktif!==false} onChange={v=>u("aktif",v)}/></div>
    ...
    <div style={kart}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px",gap:"10px",flexWrap:"wrap"}}>
        <div style={{...secBas,color:T.primary,marginBottom:0}}>👨‍👩‍👧 Yakınları <span style={{color:T.t3,fontWeight:400,fontSize:"12px"}}>({(form.kisiler||[]).length})</span></div>
        <button onClick={addYakin} style={{padding:"8px 16px",borderRadius:T.r,border:"none",background:T.primary,color:"#fff",cursor:"pointer",fontWeight:600,fontSize:"14px"}}>+ Yakın Ekle</button>
      </div>
      {(form.kisiler||[]).length===0
        ?<div style={{textAlign:"center",padding:"20px",color:T.t3,fontSize:"14px",background:"#fafafa",borderRadius:T.r,border:`1px solid ${T.border}`}}>Henüz yakın eklenmemiş (eş, çocuk, kardeş...)</div>
        :(form.kisiler||[]).map((k,i)=><div key={k.id} style={{border:`1px solid ${T.border}`,borderRadius:T.r,padding:"12px",marginBottom:"10px",background:"#fafafa"}}>
          <div style={grid}>
            <div><label style={fl}>Ad</label><input style={inp} value={k.ad||""} onChange={e=>upYakin(i,"ad",toTitleCase(e.target.value))} placeholder="Ad" onFocus={foc} onBlur={blr}/></div>
            <div><label style={fl}>Soyad</label><input style={inp} value={k.soyad||""} onChange={e=>upYakin(i,"soyad",toTitleCase(e.target.value))} placeholder="Soyad" onFocus={foc} onBlur={blr}/></div>
            <div><label style={fl}>Yakınlık</label><input style={inp} value={k.unvan||""} onChange={e=>upYakin(i,"unvan",e.target.value)} placeholder="Eş, çocuk, kardeş..." onFocus={foc} onBlur={blr}/></div>
            <div><label style={fl}>Cep</label><input style={{...inp,borderColor:phoneOk(k.telefon)?T.bDark:T.err}} value={k.telefon||""} onChange={e=>upYakin(i,"telefon",fmtPhone(e.target.value))} placeholder="0532 000 00 00" inputMode="numeric" onFocus={foc} onBlur={blr}/></div>
            <div><label style={fl}>E-posta</label><input style={inp} value={k.eposta||""} onChange={e=>upYakin(i,"eposta",e.target.value)} placeholder="ornek@mail.com" inputMode="email" onFocus={foc} onBlur={blr}/></div>
          </div>
    ...
    <div style={kart}>
      <button onClick={()=>setResmiOpen(!resmiOpen)} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",border:"none",background:"transparent",cursor:"pointer",padding:0}}>
        <span style={{...secBas,marginBottom:0,color:T.t2}}>🧾 Resmi / Vergi Bilgileri</span>
        <span style={{fontSize:"12px",color:T.t3,display:"flex",alignItems:"center",gap:"8px"}}>{!isAlici&&<span>Alıcıya dönüştürmek için gerekli</span>}<span style={{fontSize:"14px"}}>{resmiOpen?"▲":"▼"}</span></span>
      </button>
      {resmiOpen&&<div style={{marginTop:"14px",display:"flex",flexDirection:"column",gap:"12px"}}>
        <div style={{display:"flex",gap:"24px",alignItems:"center",padding:"10px 14px",background:T.pBg,borderRadius:T.r,flexWrap:"wrap"}}>
          <span style={{fontSize:"13px",fontWeight:600,color:T.t2}}>Tip:</span>
          <label style={{display:"flex",alignItems:"center",gap:"6px",cursor:"pointer",fontSize:"15px",fontWeight:form.firmaKisiTipi==="sahis"?700:400,color:form.firmaKisiTipi==="sahis"?T.primary:T.text}}><input type="radio" name="sfk_tip" checked={form.firmaKisiTipi==="sahis"} onChange={()=>{u("firmaKisiTipi","sahis");u("vergiNo","");u("vergiDairesi","");u("vergiDairesiIl","");}}/>Şahıs</label>
          <label style={{display:"flex",alignItems:"center",gap:"6px",cursor:"pointer",fontSize:"15px",fontWeight:form.firmaKisiTipi==="tuzel"?700:400,color:form.firmaKisiTipi==="tuzel"?T.primary:T.text}}><input type="radio" name="sfk_tip" checked={form.firmaKisiTipi==="tuzel"} onChange={()=>{u("firmaKisiTipi","tuzel");u("tcKimlikNo","");}}/>Tüzel Kişi</label>
        </div>
        {form.firmaKisiTipi==="sahis"
          ?<div style={grid}>
            <div><label style={fl}>TC Kimlik No <span style={{color:T.t3,fontWeight:400}}>(11 hane)</span></label><input style={{...inp,borderColor:form.tcKimlikNo&&form.tcKimlikNo.replace(/\D/g,"").length!==11?T.err:T.bDark}} value={form.tcKimlikNo||""} onChange={e=>u("tcKimlikNo",e.target.value.replace(/\D/g,"").slice(0,11))} placeholder="00000000000" inputMode="numeric" onFocus={foc} onBlur={blr}/></div>
          </div>
          :<div style={grid}>
            <div><label style={fl}>Vergi Dairesi İli</label><select style={{...inp,cursor:"pointer"}} value={form.vergiDairesiIl||""} onChange={e=>{u("vergiDairesiIl",e.target.value);u("vergiDairesi","");}} onFocus={foc} onBlur={blr}><option value="">İl seçiniz</option>{IL_LISTESI.map(il=><option key={il} value={il}>{il}</option>)}</select></div>
            <div><label style={fl}>Vergi Dairesi</label><select style={{...inp,cursor:"pointer",background:!form.vergiDairesiIl?"#f5f5f5":"#fff"}} value={form.vergiDairesi||""} onChange={e=>u("vergiDairesi",e.target.value)} disabled={!form.vergiDairesiIl} onFocus={foc} onBlur={blr}><option value="">{form.vergiDairesiIl?"Vergi dairesi seçiniz":"Önce il seçiniz"}</option>{(VERGI_DAIRELERI[form.vergiDairesiIl]||[]).map(vd=><option key={vd} value={vd}>{vd}</option>)}</select></div>
            <div><label style={fl}>Vergi No <span style={{color:T.t3,fontWeight:400}}>(10 hane)</span></label><input style={{...inp,borderColor:form.vergiNo&&form.vergiNo.replace(/\D/g,"").length!==10?T.err:T.bDark}} value={form.vergiNo||""} onChange={e=>u("vergiNo",e.target.value.replace(/\D/g,"").slice(0,10))} placeholder="0000000000" inputMode="numeric" onFocus={foc} onBlur={blr}/></div>
          </div>}
        <div style={grid}>
          <div><label style={fl}>Para Birimi</label><select style={{...inp,cursor:"pointer"}} value={form.paraBirimi||"TL"} onChange={e=>u("paraBirimi",e.target.value)} onFocus={foc} onBlur={blr}>{PARA_BIRIMLERI.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select></div>
          <div><label style={fl}>Posta Kodu</label><input style={inp} value={form.postaKodu||""} onChange={e=>u("postaKodu",e.target.value)} placeholder="00000" inputMode="numeric" onFocus={foc} onBlur={blr}/></div>
        </div>
      </div>}
    </div>
    </>}
    ...
    <div style={kart}>
      ...
      <div style={{marginBottom:"16px"}}>
        ...
        {firmaSunumlari.length===0
          ?<div style={{textAlign:"center",padding:"16px",color:T.t3,background:"#fafafa",borderRadius:T.r,border:`1px solid ${T.border}`,fontSize:"13px"}}>Müşteriye henüz sunum yapılmamış.</div>
          :<div style={{border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden"}}>{firmaSunumlari.map((s,idx)=>{const sonuc=sonucEt[s.sonuc];const ilgi=ilgiEt[s.ilgiSeviyesi];return <div key={s.id} style={{display:"flex",gap:"12px",padding:"12px 14px",background:idx%2===0?"#fff":"#fafafa",borderBottom:idx<firmaSunumlari.length-1?`1px solid ${T.border}`:"none"}}>
            <div style={{width:"66px",flexShrink:0,fontSize:"12px",fontWeight:700,color:"#722ed1",textAlign:"center",background:"#f9f0ff",borderRadius:"6px",padding:"6px 4px",height:"fit-content"}}>{s.tarih?s.tarih.split("-").reverse().join("."):"—"}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:"13px",fontWeight:600,color:T.text}}>{projeAdOf(s.projeId)}{daireAdOf(s.projeId,s.bolumId)?` • ${daireAdOf(s.projeId,s.bolumId)}`:""}</div>
              <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginTop:"4px"}}>{sonuc&&<span style={{fontSize:"11px",fontWeight:600,color:sonuc.c,background:sonuc.bg,padding:"2px 8px",borderRadius:"10px"}}>{sonuc.l}</span>}{ilgi&&<span style={{fontSize:"11px",fontWeight:600,color:ilgi.c,background:ilgi.bg,padding:"2px 8px",borderRadius:"10px"}}>{ilgi.l}</span>}</div>
              {s.notlar&&<div style={{fontSize:"12px",color:T.t2,fontStyle:"italic",marginTop:"4px"}}>"{s.notlar}"</div>}
            </div>
          </div>;})}</div>}
      </div>
    </div>
    ...
    <div style={kart}>
      ...
      <div style={{display:"flex",gap:"8px",marginBottom:"12px"}}>
        <input style={{...inp,flex:1}} value={nn} onChange={e=>setNn(e.target.value)} placeholder="Yeni not yazın..." onKeyDown={e=>e.key==="Enter"&&addN()} onFocus={foc} onBlur={blr}/>
        <button onClick={addN} style={{padding:"0 20px",borderRadius:T.r,border:"none",background:T.primary,color:"#fff",cursor:"pointer",fontWeight:600,fontSize:"14px",whiteSpace:"nowrap",flexShrink:0}}>+ Ekle</button>
      </div>
      {(form.notlar||[]).length===0
...
const FirmalarPage=({firmalar,setFirmalar,onSave,onDel,addNote,initialFirmaId,onClearInitial,projeler=[],setPage,setEditMode,kisitliTurler=null,currentUser=null,sunumlar=[],yeniAcSinyal=0})=>{
  const[search,setSearch]=useState("");
  ...
  // Dışarıdan "yeni firma" tetikleme (Satış Sunumu → Yeni Müşteri)
  useEffect(()=>{
    if(yeniAcSinyal){setView("list");setShowKod(true);}
  },[yeniAcSinyal]);
  const[initData,setInitData]=useState(null);

  const filtered=firmalar.filter(f=>{const q=nTR(search);const ms=nTR(f.ad).includes(q)||nTR(f.firmaKodu).includes(q)||nTR(f.aciklama).includes(q)||f.kisiler.some(k=>nTR(`${k.ad} ${k.soyad}`).includes(q));const mt=fTur==="all"||f.turler.includes(fTur);const md=fDurum==="all"||(fDurum==="aktif"?f.aktif!==false:f.aktif===false);const mk=!kisitliTurler||(Array.isArray(f.turler)&&f.turler.some(t=>kisitliTurler.includes(t)));return ms&&mt&&md&&mk;});
  ...
  /* FIRMA KARTI — satıcı(sales) için sadeleştirilmiş iPad kartı; diğer roller tam kart */
  const KartComp = isSatici(currentUser) ? SaticiFirmaKarti : FirmaKarti;
  if(view==="form-new"){
    return <KartComp
      firma={null}
      initData={initData}
      isNew={true}
      onSave={async(d)=>{await handleSave(d);setView("list");setInitData(null);setEditMode?.(false);}}
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:"10px",marginBottom:"16px",flexWrap:"wrap",alignItems:"center"}}>
      ...
      <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
        ...
        {FIRMA_TURLERI.filter(t=>!kisitliTurler||kisitliTurler.includes(t.id)).map(t=><button key={t.id} onClick={()=>setFTur(fTur===t.id?"all":t.id)} style={{height:"36px",padding:"0 14px",borderRadius:T.r,border:`1px solid ${fTur===t.id?"#384248":T.bDark}`,background:fTur===t.id?"#384248":"#fff",color:fTur===t.id?"#fff":T.t2,fontSize:"14px",cursor:"pointer"}}>{t.label}</button>)}
      </div>
      <div style={{display:"flex",gap:"6px"}}>
        {[{k:"aktif",l:"Aktif"},{k:"pasif",l:"Pasif"},{k:"all",l:"Tümü"}].map(d=><button key={d.k} onClick={()=>setFDurum(d.k)} style={{height:"36px",padding:"0 14px",borderRadius:T.r,border:`1px solid ${fDurum===d.k?(d.k==="aktif"?T.ok:d.k==="pasif"?T.err:"#384248"):T.bDark}`,background:fDurum===d.k?(d.k==="aktif"?"#f6ffed":d.k==="pasif"?"#fff1f0":"#384248"):"#fff",color:fDurum===d.k?(d.k==="aktif"?T.ok:d.k==="pasif"?T.err:"#fff"):T.t2,fontSize:"14px",cursor:"pointer",fontWeight:fDurum===d.k?600:400}}>{d.l}</button>)}
      </div>
...
const projeToLocal = (p) => ({
  ...
  anlasmaYontemi: p.anlasma_yontemi||"", arsaSahibiPay: p.arsa_sahibi_pay||"", muteahhitPay: p.muteahhit_pay||"", maliyetMuteahhitPay: p.maliyet_muteahhit_pay!=null?String(p.maliyet_muteahhit_pay):"", maliyetArsaSahibiPay: p.maliyet_arsa_sahibi_pay!=null?String(p.maliyet_arsa_sahibi_pay):"",
  aciklama: p.aciklama||"",
...
const projeToDb = (p) => ({
  ...
  anlasma_yontemi: p.anlasmaYontemi||"", arsa_sahibi_pay: p.arsaSahibiPay||"", muteahhit_pay: p.muteahhitPay||"", maliyet_muteahhit_pay: p.maliyetMuteahhitPay!==""&&p.maliyetMuteahhitPay!=null?Number(p.maliyetMuteahhitPay):null, maliyet_arsa_sahibi_pay: p.maliyetArsaSahibiPay!==""&&p.maliyetArsaSahibiPay!=null?Number(p.maliyetArsaSahibiPay):null,
  aciklama: p.aciklama||"",
...
// Bütçe kalemi satırı (butce_kalemi_satirlari ayrı tablosu — JSONB'den ayrılmış)
const kalemSatirToLocal = (s) => ({
  id: s.id,
  butceKalemiId: s.butce_kalemi_id,
  siraNo: s.sira_no!=null?String(s.sira_no):"",
  aciklama: s.aciklama||"",
  miktar: s.miktar!=null?String(s.miktar):"",
  guncelMiktar: s.guncel_miktar!=null?String(s.guncel_miktar):"",
  birimFiyat: s.birim_fiyat!=null?String(s.birim_fiyat):"",
  guncelBirimFiyat: s.guncel_birim_fiyat!=null?String(s.guncel_birim_fiyat):"",
  kdvOrani: s.kdv_orani||"20",
  tamamlandi: s.tamamlandi===true
});
const kalemSatirToDb = (s, butceKalemiId) => ({
  butce_kalemi_id: butceKalemiId,
  sira_no: s.siraNo!==""&&s.siraNo!=null?parseInt(s.siraNo)||0:0,
  aciklama: s.aciklama||"",
  miktar: s.miktar!==""&&s.miktar!=null?Number(s.miktar):null,
  guncel_miktar: s.guncelMiktar!==""&&s.guncelMiktar!=null?Number(s.guncelMiktar):(s.miktar!==""&&s.miktar!=null?Number(s.miktar):null),
  birim_fiyat: s.birimFiyat!==""&&s.birimFiyat!=null?Number(s.birimFiyat):null,
  guncel_birim_fiyat: s.guncelBirimFiyat!==""&&s.guncelBirimFiyat!=null?Number(s.guncelBirimFiyat):null,
  kdv_orani: s.kdvOrani||"20",
  tamamlandi: s.tamamlandi===true
});
// Satır revizyonu (butce_kalemi_satir_revizyonlari)
const satirRevToLocal = (r) => ({
  id: r.id,
  butceKalemiSatirId: r.butce_kalemi_satir_id,
  revNo: r.rev_no||0,
  tarih: r.tarih||"",
  tip: r.tip||"fiyat",
  eskiDeger: r.eski_deger!=null?String(r.eski_deger):"",
  yeniDeger: r.yeni_deger!=null?String(r.yeni_deger):"",
  aciklama: r.aciklama||"",
  createdAt: r.created_at||""
});
const satirRevToDb = (r, satirId) => ({
  butce_kalemi_satir_id: satirId,
  rev_no: r.revNo||0,
  tarih: r.tarih||new Date().toISOString().split("T")[0],
  tip: r.tip||"fiyat",
  eski_deger: r.eskiDeger!==""&&r.eskiDeger!=null?Number(r.eskiDeger):null,
  yeni_deger: r.yeniDeger!==""&&r.yeniDeger!=null?Number(r.yeniDeger):null,
  aciklama: r.aciklama||""
});

// Alt satır otomatik kapanma kontrolü — butceKalemiSatirId eşleşen faturalar miktar/fiyat birebir tutturursa kapanır
const altSatirEslesti = (altSatir, bkId, faturalarList) => {
  if(!altSatir || !altSatir.id) return false;
  const pm = parseFloat(altSatir.miktar)||0;
  const pf = parseFloat(altSatir.birimFiyat)||0;
  if(pm<=0 || pf<=0) return false;
  const activeF = (faturalarList||[]).filter(f=>f.durum!=='iptal');
  const fkAll = activeF.flatMap(f=>(f.kalemler||[]).filter(fk=>
    String(fk.butceKalemiId)===String(bkId) &&
    fk.butceKalemiSatirId &&
    String(fk.butceKalemiSatirId)===String(altSatir.id)
  ));
...
// SP durum hesabı — faturalar üzerinden; saveFatura + backfill ortak kullanır
const computeSpDurumu = (sp, faturalarList) => {
  if(!sp) return null;
  if(sp.durum==='iptal'||sp.durum==='tamamlandi') return sp.durum;
  const activeFaturalar = (faturalarList||[]).filter(f=>f.durum!=='iptal');
  const spKalemleri = sp.kalemler||[];
  if(spKalemleri.length===0) return sp.durum;
  let tumKapali = true;
  let enAzBir = false;
  for(const sk of spKalemleri) {
    const skMiktar = parseFloat(sk.miktar)||0;
    if(skMiktar<=0){tumKapali=false;continue;}
    const faturalanan = activeFaturalar.reduce((t,f)=>{
      return t + (f.kalemler||[]).filter(fk=>{
        // 1) Alt satır birebir eşleşme
        if(sk.butceKalemiSatirId && fk.butceKalemiSatirId && String(fk.butceKalemiSatirId)===String(sk.butceKalemiSatirId)) return true;
        // 2) Bütçe kalemi + malzeme eşleşmesi (karışık SP faturalarında)
        if(sk.butceKalemiId && fk.butceKalemiId && String(fk.butceKalemiId)===String(sk.butceKalemiId) && sk.malzemeId && fk.malzemeId && String(fk.malzemeId)===String(sk.malzemeId)) return true;
        // 3) Legacy: spId doğrudan bağı + malzeme
        if(f.spId && String(f.spId)===String(sp.id) && sk.malzemeId && fk.malzemeId && String(fk.malzemeId)===String(sk.malzemeId)) return true;
        return false;
      }).reduce((s,fk)=>s+(parseFloat(fk.miktar)||0),0);
    }, 0);
    if(faturalanan>0) enAzBir=true;
    if(faturalanan<skMiktar) tumKapali=false;
  }
  if(tumKapali) return 'tamamlandi';
  if(enAzBir) return 'kismen_teslim';
  return sp.durum;
};
...
const bolumToLocal = (b) => ({
  ...
  plus: b.plus!=null?String(b.plus):"",
...
const bolumToDb = (b, projeId) => ({
  ...
  plus: b.plus!==""&&b.plus!=null?Number(b.plus):null,
...
// SUNUMLAR — opsiyon öncesi ilk görüşme / lead kaydı (daireyi kilitlemez, 1 daire → N sunum)
const sunumToLocal = (s) => ({
  id:s.id, projeId:s.proje_id, bolumId:s.bolum_id,
  firmaId:s.firma_id, firmaAd:s.firma_ad||"", telefon:s.telefon||"",
...
const sunumToDb = (s) => ({
  proje_id:s.projeId||null, bolum_id:s.bolumId||null,
  firma_id:s.firmaId||null, firma_ad:s.firmaAd||"", telefon:s.telefon||"",
  tarih:s.tarih||null,
    ...
    <div style={{background:"#fff",borderRadius:T.rl,width:"1000px",height:"800px",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",padding:"20px"}}>
        {tab==="bilgi"&&<div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px"}}>
            <div><label style={lS}>Satış Bedeli <span style={{color:T.t3,fontSize:"11px"}}>(KDV Hariç, Fatura)</span></label><input style={iS} value={form.satisBedeli?Number(form.satisBedeli).toLocaleString("tr-TR"):""} onChange={e=>{const v=e.target.value.replace(/\./g,"").replace(/[^0-9]/g,"");u("satisBedeli",v);}} placeholder="0" onFocus={foc} onBlur={blr}/></div>
            <div><label style={lS}>PLUS</label><input style={{...iS,background:"#fffbe6",color:"#d48806",fontWeight:600}} value={form.plus?Number(form.plus).toLocaleString("tr-TR"):""} onChange={e=>{const v=e.target.value.replace(/\./g,"").replace(/[^0-9]/g,"");u("plus",v);}} placeholder="0" onFocus={foc} onBlur={blr}/></div>
          </div>
...
const ProjeKarti=({proje,isNew,onSave,onDel,onBack,firmalar,setPage:setMainPage,goToFirma,malzemeler=[],bagimsizBolumKartiAc,tumMuteahhitDairelerineKartAc})=>{
  const emptyProje={
    ...
    anlasmaYontemi:"",arsaSahibiPay:"",muteahhitPay:"",maliyetMuteahhitPay:"",maliyetArsaSahibiPay:"",
    aciklama:"",
    firmaBaglantilari:[],
    bloklar:[],
    bolumler:[],
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"24px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
            ...
            {form.anlasmaYontemi&&<>
              ...
              {/* MANUEL KAT KARŞILIĞI ORANI — maliyet hesabı için manuel girilen ayrı oran */}
              <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:"12px",alignItems:"center"}}>
                <label style={{fontSize:"13px",fontWeight:600,color:T.text,textAlign:"right",height:"36px",lineHeight:"36px"}} title="Maliyet hesaplamalarında kullanılacak müteahhit oranı. Boş bırakırsanız yukarıdaki Müteahhit Payı kullanılır.">Manuel Kat Karşılığı Oranı (Müteahhit)</label>
                <input style={{...iS,fontWeight:700,color:"#722ed1"}} value={form._malMutPayEdit?form.maliyetMuteahhitPay||"":(form.maliyetMuteahhitPay?form.maliyetMuteahhitPay+" %":"")} onChange={e=>{const v=e.target.value.replace(/[^0-9.,]/g,"").replace(",",".");u("maliyetMuteahhitPay",v);}} placeholder={form.muteahhitPay?`Boş → ${form.muteahhitPay}% kullanılır`:"0"} onFocus={()=>u("_malMutPayEdit",true)} onBlur={()=>u("_malMutPayEdit",false)}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:"12px",alignItems:"center"}}>
                <label style={{fontSize:"13px",fontWeight:600,color:T.text,textAlign:"right",height:"36px",lineHeight:"36px"}} title="Maliyet hesaplamalarında kullanılacak arsa sahibi oranı. Müteahhit ile toplamı %100 olmalıdır.">Manuel Kat Karşılığı Oranı (Arsa Sahibi)</label>
                <input style={{...iS,fontWeight:700,color:"#722ed1"}} value={form._malAsPayEdit?form.maliyetArsaSahibiPay||"":(form.maliyetArsaSahibiPay?form.maliyetArsaSahibiPay+" %":"")} onChange={e=>{const v=e.target.value.replace(/[^0-9.,]/g,"").replace(",",".");u("maliyetArsaSahibiPay",v);}} placeholder={form.arsaSahibiPay?`Boş → ${form.arsaSahibiPay}% kullanılır`:"0"} onFocus={()=>u("_malAsPayEdit",true)} onBlur={()=>u("_malAsPayEdit",false)}/>
              </div>
              {/* TOPLAM KONTROLÜ — sadece her iki alan da doluysa göster */}
              {(form.maliyetMuteahhitPay||form.maliyetArsaSahibiPay)&&(()=>{
                const mut=parseFloat(form.maliyetMuteahhitPay)||0;
                const as=parseFloat(form.maliyetArsaSahibiPay)||0;
                const toplam=mut+as;
                const ok=Math.abs(toplam-100)<0.01;
                return <div style={{display:"grid",gridTemplateColumns:"260px 1fr",gap:"12px",alignItems:"center"}}>
                  <div></div>
                  <div style={{fontSize:"12px",fontWeight:600,color:ok?"#52c41a":"#ff4d4f",padding:"6px 10px",borderRadius:T.r,background:ok?"#f6ffed":"#fff1f0",border:`1px solid ${ok?"#b7eb8f":"#ffa39e"}`}}>{ok?`✓ Toplam %100 — geçerli`:`⚠ Toplam %${toplam.toFixed(2)} — Müteahhit + Arsa Sahibi = %100 olmalı`}</div>
                </div>;
              })()}
            </>}
            {/* AYIRICI */}
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:"#384248",flexWrap:"wrap",gap:"8px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"20px"}}>
                ...
                <button onClick={()=>{
                  ...
                  blokBolumler.forEach(b=>{const ds=BOLUM_DURUMLARI.find(d=>d.id===b.durum);const net=(parseFloat(b.satisBedeli)||0)+(parseFloat(b.plus)||0);rows.push([b.no||"",b.kat||"",b.tipi||"",b.brutM2||"",b.netM2||"",b.odaSayisi||"",b.cephe||"",net||"",ds?ds.label:""]);});
                  rows.push(["","","TOPLAM",topBrut,topNet,"","",blokBolumler.reduce((s,b)=>s+(parseFloat(b.satisBedeli)||0)+(parseFloat(b.plus)||0),0),""]);
                  const csv=rows.map(r=>r.map(c=>`"${c}"`).join(";")).join("\n");
                  const blob=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"});
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"16px",flexWrap:"wrap",gap:"12px"}}>
            <div style={{display:"flex",gap:"16px",flexWrap:"wrap",alignItems:"center"}}>
              {BOLUM_DURUMLARI.map(d=>{
                ...
                const topFiyat=bolumler.filter(b=>b.durum===d.id).reduce((s,b)=>s+(parseFloat(b.satisBedeli)||0)+(parseFloat(b.plus)||0),0);
...
const ButceKalemModal=({kalem,onSave,onDel,onClose,malzemeler,projeBloklar=[],projeBolumler=[],ortakAlanM2="",siparisler=[],faturalar=[],firmalar=[]})=>{
  ...
  const[revForm,setRevForm]=useState({satirId:"",tip:"fiyat",yeniDeger:"",aciklama:""});
  const addRevizyon=()=>{
    if(!revForm.yeniDeger||!revForm.satirId)return;
    const satirIdx=(fm.planlananSatirlari||[]).findIndex(s=>String(s.id)===String(revForm.satirId));
    if(satirIdx<0)return;
    const satir=fm.planlananSatirlari[satirIdx];
    const eskiDeger=revForm.tip==="fiyat"?(satir.guncelBirimFiyat||satir.birimFiyat||"0"):(satir.guncelMiktar||satir.miktar||"0");
    const revNo=(satir.revizyonlar||[]).length+1;
    const yeniRev={id:Date.now(),butceKalemiSatirId:satir.id,tarih:new Date().toISOString().split("T")[0],tip:revForm.tip,eskiDeger:String(eskiDeger),yeniDeger:revForm.yeniDeger,aciklama:revForm.aciklama||"",revNo};
    const newSatirlar=(fm.planlananSatirlari||[]).map((s,i)=>{
      if(i!==satirIdx)return s;
      const updated={...s,revizyonlar:[...(s.revizyonlar||[]),yeniRev]};
      if(revForm.tip==="fiyat")updated.guncelBirimFiyat=revForm.yeniDeger;
      else updated.guncelMiktar=revForm.yeniDeger;
      return updated;
    });
    uf("planlananSatirlari",newSatirlar);
    setRevForm({satirId:"",tip:"fiyat",yeniDeger:"",aciklama:""});setRevEkleAcik(false);
  };
  const delRevizyon=(satirId,revId)=>{
    const newSatirlar=(fm.planlananSatirlari||[]).map(s=>String(s.id)===String(satirId)?{...s,revizyonlar:(s.revizyonlar||[]).filter(r=>r.id!==revId)}:s);
    uf("planlananSatirlari",newSatirlar);
  };
  const saveRevDuzenle=(satirId,revId,field,val)=>{
    const newSatirlar=(fm.planlananSatirlari||[]).map(s=>String(s.id)===String(satirId)?{...s,revizyonlar:(s.revizyonlar||[]).map(r=>r.id===revId?{...r,[field]:val}:r)}:s);
    uf("planlananSatirlari",newSatirlar);
  };
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,overflow:"auto",display:"grid",placeContent:"center",padding:"20px"}}>
    <div style={{background:"#fff",borderRadius:T.rl,width:"2100px",height:"1000px",display:"flex",flexDirection:"column",overflow:"hidden",fontFamily:"'Arial Narrow', Arial, sans-serif"}}>
      <div style={{padding:"12px 20px",background:"#384248",display:"flex",alignItems:"center",gap:"16px",borderRadius:`${T.rl} ${T.rl} 0 0`}}>
        <button onClick={onClose} title="Kapat" style={{padding:"0",border:"none",background:"transparent",color:"#8799a3",cursor:"pointer",display:"flex",alignItems:"center"}}><MoveLeft size={28}/></button>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"20px"}}>
          <button onClick={()=>{if(!kaydetValidasyon())return;const pt=fiyatToplamHesapla(fm.planlananSatirlari);const cleanedSatirlar=(fm.planlananSatirlari||[]).map(s=>{const{_isNew,_miktarEdit,_bfEdit,_gbfEdit,...rest}=s;return rest;});onSave({...fm,planlananSatirlari:cleanedSatirlar,planlananToplam:pt.kdvHaricTop,planlananMiktar:String(pt.miktarTop||""),planlananBirimFiyat:(pt.miktarTop>0?String(pt.kdvHaricTop/pt.miktarTop):"")});setFm(p=>({...p,planlananSatirlari:cleanedSatirlar}));setSaved(true);setTimeout(()=>setSaved(false),2000);setHatalar({});}} title={saved?"Kaydedildi":"Kaydet"} style={{padding:"0",border:"none",background:"transparent",color:saved?"#52c41a":"#8799a3",cursor:"pointer",display:"flex",alignItems:"center",transition:"color .3s"}}><Save size={30}/></button>
      ...
      <div style={{flex:1,overflow:"auto",padding:"20px",display:"flex",flexDirection:"column",gap:"14px"}}>
        ...
        {(()=>{
          ...
          const addSatir=()=>{
            ...
            uf("planlananSatirlari",[...satirlar,{id:yeniId,_isNew:true,siraNo:String(satirlar.length+1),aciklama:"",miktar:"",guncelMiktar:"",birimFiyat:"",guncelBirimFiyat:"",kdvOrani:varsayilanKdv,tamamlandi:false}]);
          };
          const delSatir=(idx)=>{
          ...
          const upPlan=(idx,field,val)=>{
            const arr=[...satirlar];
            const old=arr[idx];
            // B.Fiyat / Miktar değişince auto-sync: güncel değer henüz manuel değiştirilmediyse planlanan ile aynı tutulur
            if(field==="birimFiyat"){
              const oldP=old.birimFiyat||"";
              const oldG=old.guncelBirimFiyat||"";
              arr[idx]={...old,birimFiyat:val};
              if(oldG===""||oldG===oldP){ arr[idx].guncelBirimFiyat=val; }
            } else if(field==="miktar"){
              const oldP=old.miktar||"";
              const oldG=old.guncelMiktar||"";
              arr[idx]={...old,miktar:val};
              if(oldG===""||oldG===oldP){ arr[idx].guncelMiktar=val; }
            } else {
              arr[idx]={...old,[field]:val};
            }
            uf("planlananSatirlari",arr);
          };
          ...
          let pMikTop=0,gMikTop=0,pKdvHTop=0,pKdvTop=0,gKdvHTop=0,gKdvTop=0;
          satirlar.forEach(s=>{
            const m=parseFloat(s.miktar||0);
            const mG=parseFloat(s.guncelMiktar||s.miktar||0);
            const pBf=parseFloat(s.birimFiyat||0);
            const gBf=parseFloat(s.guncelBirimFiyat||0);
            const pKdvO=parseInt(s.kdvOrani||varsayilanKdv)/100;
            const pH=m*pBf;
            const gH=mG*gBf;
            pMikTop+=m;gMikTop+=mG;
            pKdvHTop+=pH;pKdvTop+=pH*pKdvO;
            gKdvHTop+=gH;gKdvTop+=gH*pKdvO;
          });

          const gridCols="30px 32px 350px 65px 50px 50px 16px 65px 75px 75px 85px 16px 65px 75px 75px 85px 33px";

          return <div style={{display:"flex",gap:"30px",alignItems:"flex-start"}}>
          <div style={{border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden",flex:"0 0 auto"}}>
            {/* HESAPLA BUTONU */}
            ...
            {/* BAŞLIK */}
            <div style={{display:"grid",gridTemplateColumns:gridCols,gap:"4px",padding:"6px 8px",alignItems:"center"}}>
              <div style={{gridColumn:"span 6",border:`1px solid ${T.bDark}`,borderRadius:"8px",padding:"6px 8px",textAlign:"center",fontSize:"14pt",fontWeight:700,color:"#8799a3",background:"#384248"}}>GENEL BİLGİLER</div>
              <div></div>
              <div style={{gridColumn:"span 4",border:`1px solid ${T.bDark}`,borderRadius:"8px",padding:"6px 8px",textAlign:"center",fontSize:"14pt",fontWeight:700,color:"rgb(255,245,235)",background:"rgb(255,128,64)"}}>PLANLANAN HESAPLAMALAR</div>
              <div></div>
              <div style={{gridColumn:"span 4",border:`1px solid ${T.bDark}`,borderRadius:"8px",padding:"6px 8px",textAlign:"center",fontSize:"14pt",fontWeight:700,color:"rgb(230,245,245)",background:"rgb(0,128,128)"}}>GÜNCEL HESAPLAMALAR</div>
              <button onClick={addSatir} title="Satır Ekle" style={{padding:"0",border:"none",background:"transparent",color:"#8799a3",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><SquarePlus size={30}/></button>
            </div>
            {/* TABLO HEADER */}
            <div style={{display:"grid",gridTemplateColumns:gridCols,gap:"4px",padding:"4px 8px",borderBottom:`1px solid ${T.border}`}}>
              {["✓","S.No","Açıklama","Miktar","Birim","KDV%","","B.Fiyat","KDV Hariç","KDV Tut.","Toplam","","B.Fiyat","KDV Hariç","KDV Tut.","Toplam",""].map((h,i)=><div key={i} style={{fontSize:"9pt",fontWeight:500,color:i>=12?"rgb(0,128,128)":(i>=7?"rgb(255,128,64)":"rgb(128,128,128)"),textTransform:"uppercase",textAlign:"center"}}>{h}</div>)}
            </div>
            {/* SATIRLAR */}
            <div style={{maxHeight:"400px",overflow:"auto"}}>
              {satirlar.length===0
                ?<div style={{padding:"24px",textAlign:"center",color:T.t3,fontSize:"12px"}}>Satır eklemek için sağ üstteki + butonuna tıklayın.</div>
                :satirlar.map((s,idx)=>{
                  const m=parseFloat(s.miktar||0);
                  const mG=parseFloat(s.guncelMiktar||s.miktar||0);
                  ...
                  const gBf=parseFloat(s.guncelBirimFiyat||0);
                  const gH=mG*gBf;const gKdvT=gH*(pKdvO/100);const gTop=gH+gKdvT;
                  ...
                  return <div key={s.id} style={{display:"grid",gridTemplateColumns:gridCols,gap:"4px",padding:"3px 8px",alignItems:"center",borderBottom:idx<satirlar.length-1?`1px solid ${T.border}`:"none",background:rowBg}}>
                    ...
                    <input style={{...iS,fontSize:"10pt",padding:"4px 4px",textAlign:"center",color:altTxtClr||T.text,height:"30px",lineHeight:"30px"}} value={s.siraNo||String(idx+1)} onChange={e=>upPlan(idx,"siraNo",e.target.value)} onFocus={foc} onBlur={blr}/>
                    <input style={{...iS,fontSize:"10pt",padding:"3px 3px",height:"30px",lineHeight:"30px"}} value={s.aciklama||""} onChange={e=>upPlan(idx,"aciklama",e.target.value)} placeholder="Açıklama..." onFocus={foc} onBlur={blr}/>
                    <input disabled={!isNew} title={isNew?"":"Kayıtlı — değişiklik için Revizyon Ekle butonu"} style={{...iS,fontSize:"10pt",padding:"4px 4px",textAlign:"right",fontWeight:400,height:"30px",lineHeight:"30px",...(isNew?{}:{background:lockBg,cursor:"not-allowed"})}} value={isNew?(s._miktarEdit?s.miktar||"":(parseFloat(s.miktar||0)>0?parseFloat(s.miktar).toLocaleString("tr-TR"):"")):(mG>0?mG.toLocaleString("tr-TR"):"")} onChange={e=>{if(!isNew)return;const raw=e.target.value.replace(/[^0-9.,]/g,"").replace(/,/g,".");const pts=raw.split(".");const v=pts.length>2?pts[0]+"."+pts.slice(1).join(""):raw;upPlan(idx,"miktar",v);}} placeholder="0" onFocus={e=>{if(!isNew)return;upPlan(idx,"_miktarEdit",true);foc(e);}} onBlur={e=>{if(!isNew)return;upPlan(idx,"_miktarEdit",false);blr(e);}}/>
                    <div style={{fontSize:"9pt",color:T.t2,textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box"}}>{birim}</div>
                    <select style={{...iS,fontSize:"9pt",padding:"2px",textAlign:"center",height:"30px",lineHeight:"30px"}} value={s.kdvOrani||varsayilanKdv} onChange={e=>upPlan(idx,"kdvOrani",e.target.value)}>{KDV_ORANLARI.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select>
                    <div></div>
                    <input style={{...iS,fontSize:"10pt",padding:"4px 4px",textAlign:"right",fontWeight:600,height:"30px",lineHeight:"30px",color:"rgb(255,128,64)",background:"rgb(255,245,235)"}} value={s._bfEdit?s.birimFiyat||"":(parseFloat(s.birimFiyat||0)>0?parseFloat(s.birimFiyat).toLocaleString("tr-TR"):"")} onChange={e=>{const raw=e.target.value.replace(/[^0-9.,]/g,"").replace(/,/g,".");const pts=raw.split(".");const v=pts.length>2?pts[0]+"."+pts.slice(1).join(""):raw;upPlan(idx,"birimFiyat",v);}} placeholder="0" onFocus={e=>{upPlan(idx,"_bfEdit",true);foc(e);}} onBlur={e=>{upPlan(idx,"_bfEdit",false);blr(e);}}/>
                    <div style={{fontSize:"10pt",fontWeight:400,color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(255,245,235)"}}>{pH>0?pH.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <div style={{fontSize:"10pt",color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(255,245,235)"}}>{pKdvT>0?pKdvT.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <div style={{fontSize:"11.5pt",fontWeight:600,color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(255,245,235)"}}>{pTop>0?pTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <div></div>
                    <input disabled={!isNew} title={isNew?"":"Kayıtlı — değişiklik için Revizyon Ekle butonu"} style={{...iS,fontSize:"10pt",padding:"4px 4px",textAlign:"right",fontWeight:600,height:"30px",lineHeight:"30px",color:"rgb(0,128,128)",background:isNew?"rgb(230,245,245)":lockBg,...(isNew?{}:{cursor:"not-allowed"})}} value={s._gbfEdit?s.guncelBirimFiyat||"":(parseFloat(s.guncelBirimFiyat||0)>0?parseFloat(s.guncelBirimFiyat).toLocaleString("tr-TR"):"")} onChange={e=>{if(!isNew)return;const raw=e.target.value.replace(/[^0-9.,]/g,"").replace(/,/g,".");const pts=raw.split(".");const v=pts.length>2?pts[0]+"."+pts.slice(1).join(""):raw;upPlan(idx,"guncelBirimFiyat",v);}} placeholder="0" onFocus={e=>{if(!isNew)return;upPlan(idx,"_gbfEdit",true);foc(e);}} onBlur={e=>{if(!isNew)return;upPlan(idx,"_gbfEdit",false);blr(e);}}/>
                    <div style={{fontSize:"10pt",fontWeight:400,color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,245,245)"}}>{gH>0?gH.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <div style={{fontSize:"10pt",color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,245,245)"}}>{gKdvT>0?gKdvT.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <div style={{fontSize:"11.5pt",fontWeight:600,color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,245,245)"}}>{gTop>0?gTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    <button onClick={()=>delSatir(idx)} style={{padding:"0",border:"none",background:"transparent",color:T.err,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",width:"33px",height:"33px"}}><Trash2 size={20}/></button>
                  </div>;
                })
              }
            </div>
            ...
            {satirlar.length>0&&<div style={{display:"grid",gridTemplateColumns:gridCols,gap:"4px",padding:"6px 8px",background:"#f0f5ff",borderTop:`2px solid ${T.primary}`}}>
              <div></div>
              <div></div>
              <div style={{fontSize:"12pt",fontWeight:700,color:T.text,textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>TOPLAM</div>
              <div style={{fontSize:"12pt",fontWeight:700,color:T.text,textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gMikTop>0?gMikTop.toLocaleString("tr-TR"):""}</div>
              <div style={{fontSize:"10pt",fontWeight:700,color:T.t2,textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{birim}</div>
              <div></div>
              <div></div>
              <div></div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{pKdvHTop>0?pKdvHTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{pKdvTop>0?pKdvTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(255,128,64)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{(pKdvHTop+pKdvTop)>0?(pKdvHTop+pKdvTop).toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div></div>
              <div></div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gKdvHTop>0?gKdvHTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gKdvTop>0?gKdvTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,128,128)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{(gKdvHTop+gKdvTop)>0?(gKdvHTop+gKdvTop).toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              <div></div>
            </div>}
            {/* AÇIKLAMA + REVİZYON TARİHÇESİ ALT SATIRI */}
            <div style={{display:"grid",gridTemplateColumns:gridCols,gap:"4px",padding:"8px 8px",borderTop:`1px solid ${T.border}`,alignItems:"flex-start"}}>
              {/* AÇIKLAMA — GENEL BİLGİLER altında (span 6) */}
              <textarea value={fm.aciklama||""} onChange={e=>uf("aciklama",e.target.value)} placeholder="Notlar..." onFocus={foc} onBlur={blr} style={{gridColumn:"span 6",fontSize:"10pt",padding:"6px 8px",border:`1px solid ${T.bDark}`,borderRadius:T.r,width:"100%",minHeight:"120px",resize:"none",boxSizing:"border-box",outline:"none",fontFamily:"inherit",color:T.text,textAlign:"left",verticalAlign:"top"}}/>
              <div></div>
              {/* REVİZYON TARİHÇESİ — PLANLANAN+GÜNCEL altında (span 9) */}
              <div style={{gridColumn:"span 9",border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden"}}>
                ...
                {revEkleAcik&&<div style={{padding:"12px",background:"#fff7e6",borderBottom:`1px solid ${T.border}`,display:"flex",flexDirection:"column",gap:"8px"}}>
                  <div style={{display:"flex",gap:"16px",alignItems:"center",flexWrap:"wrap"}}>
                    <span style={{fontSize:"12px",fontWeight:600,color:T.t2}}>Satır:</span>
                    <select style={{...iS,height:"32px",lineHeight:"32px",fontSize:"12px",maxWidth:"320px"}} value={revForm.satirId} onChange={e=>setRevForm(p=>({...p,satirId:e.target.value}))}>
                      <option value="">— Satır seçin —</option>
                      {(fm.planlananSatirlari||[]).map(s=><option key={s.id} value={s.id}>S.No {s.siraNo||"?"} — {s.aciklama||"(boş)"}</option>)}
                    </select>
                    ...
                    {revForm.satirId&&(()=>{
                      const sec=(fm.planlananSatirlari||[]).find(s=>String(s.id)===String(revForm.satirId));
                      if(!sec)return null;
                      const mev=revForm.tip==="fiyat"?(sec.guncelBirimFiyat||sec.birimFiyat||"0"):(sec.guncelMiktar||sec.miktar||"0");
                      return <span style={{fontSize:"12px",color:T.t3,marginLeft:"8px"}}>Mevcut: {Number(mev).toLocaleString("tr-TR")}</span>;
                    })()}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:"8px",alignItems:"center"}}>
                    ...
                    <input style={iS} value={revForm.aciklama} onChange={e=>setRevForm(p=>({...p,aciklama:e.target.value}))} placeholder="Açıklama..." onFocus={foc} onBlur={blr}/>
                    <button onClick={addRevizyon} disabled={!revForm.satirId||!revForm.yeniDeger} style={{padding:"6px 16px",borderRadius:T.r,border:"none",background:revForm.satirId&&revForm.yeniDeger?T.primary:"#d9d9d9",color:"#fff",fontSize:"13px",cursor:revForm.satirId&&revForm.yeniDeger?"pointer":"not-allowed",fontWeight:600}}>Ekle</button>
                  </div>
                </div>}
                {(()=>{
                  const allRevs=[];
                  (fm.planlananSatirlari||[]).forEach(s=>{
                    (s.revizyonlar||[]).forEach(r=>{
                      allRevs.push({...r,_satirId:s.id,_satirSiraNo:s.siraNo||"?",_satirAciklama:s.aciklama||""});
                    });
                  });
                  allRevs.sort((a,b)=>(a.tarih||"").localeCompare(b.tarih||"")||(a.revNo||0)-(b.revNo||0));
                  if(allRevs.length===0&&!revEkleAcik)return <div style={{padding:"16px",textAlign:"center",color:T.t3,fontSize:"13px",background:"#fff"}}>Henüz revizyon yok.</div>;
                  if(allRevs.length===0)return null;
                  return <>
                    <div style={{display:"grid",gridTemplateColumns:"40px 80px 140px 60px 90px 90px 1fr 50px",gap:"6px",padding:"4px 12px",background:"#fafafa",borderBottom:`1px solid ${T.border}`}}>
                      {["Rev","Tarih","Satır","Tip","Eski","Yeni","Açıklama",""].map((h,i)=><div key={i} style={{fontSize:"11px",fontWeight:600,color:T.t3,textTransform:"uppercase"}}>{h}</div>)}
                    </div>
                    {allRevs.map((r,idx)=><div key={r.id} style={{display:"grid",gridTemplateColumns:"40px 80px 140px 60px 90px 90px 1fr 50px",gap:"6px",padding:"4px 12px",alignItems:"center",borderBottom:idx<allRevs.length-1?`1px solid ${T.border}`:"none",background:idx%2===0?"#fff":"#fafafa"}}>
                      ...
                      <div style={{fontSize:"12px",color:T.t2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={`S.No ${r._satirSiraNo} — ${r._satirAciklama}`}>S.No {r._satirSiraNo} — {r._satirAciklama}</div>
                      ...
                      <div style={{fontSize:"13px",color:T.t3,textAlign:"right"}}>{Number(r.eskiDeger||0).toLocaleString("tr-TR")}</div>
                      <div style={{fontSize:"13px",fontWeight:600,textAlign:"right",color:parseFloat(r.yeniDeger||0)>parseFloat(r.eskiDeger||0)?"#ff4d4f":"#52c41a"}}>{Number(r.yeniDeger||0).toLocaleString("tr-TR")}</div>
                      <input style={{...iS,fontSize:"12px",height:"28px",lineHeight:"28px"}} value={r.aciklama||""} onChange={e=>saveRevDuzenle(r._satirId,r.id,"aciklama",e.target.value)} placeholder="Açıklama..." onFocus={foc} onBlur={blr}/>
                      <div style={{display:"flex",gap:"8px"}}>
                        <button onClick={()=>delRevizyon(r._satirId,r.id)} title="Sil" style={{padding:"0",border:"none",background:"transparent",color:T.err,cursor:"pointer",display:"flex",alignItems:"center"}}><Trash2 size={16}/></button>
                      </div>
                    </div>)}
                  </>;
                })()}
          ...
          {/* GERÇEKLEŞME / ALIŞ FATURALARI PORTALI */}
          {(()=>{
            const gerCols="80px 80px 130px 60px 75px 75px 75px 90px";
            const gerKalemler=[];
            (bagliAf||[]).forEach(f=>{
              (f.kalemler||[]).forEach(k=>{
                if(String(k.butceKalemiId)===String(fm.id)){
                  gerKalemler.push({...k,faturaTarihi:f.faturaTarihi,afNo:f.afNo,firmaAd:f.firmaAd});
                }
              });
            });
            let gerMikTop=0,gerKdvHTop=0,gerKdvTop=0;
            gerKalemler.forEach(k=>{
              const m=parseFloat(k.miktar||0);
              const bf=parseFloat(k.netFiyat||0);
              const kdvO=parseInt(k.kdvOrani||"20")/100;
              const kdvHar=m*bf;
              gerMikTop+=m;gerKdvHTop+=kdvHar;gerKdvTop+=kdvHar*kdvO;
            });
            return <div style={{border:`1px solid ${T.border}`,borderRadius:T.r,overflow:"hidden",flex:"0 0 auto"}}>
              {/* BAŞLIK */}
              <div style={{display:"grid",gridTemplateColumns:gerCols,gap:"4px",padding:"6px 8px",alignItems:"center"}}>
                <div style={{gridColumn:"span 8",border:`1px solid ${T.bDark}`,borderRadius:"8px",padding:"6px 8px",textAlign:"center",fontSize:"14pt",fontWeight:700,color:"rgb(230,240,250)",background:"rgb(0,110,220)"}}>GERÇEKLEŞME / ALIŞ FATURALARI</div>
              </div>
              {/* TABLO HEADER */}
              <div style={{display:"grid",gridTemplateColumns:gerCols,gap:"4px",padding:"4px 8px",borderBottom:`1px solid ${T.border}`}}>
                {["Tarih","AF No","Firma","Miktar","B.Fiyat","KDV Hariç","KDV Tut.","Toplam"].map((h,i)=><div key={i} style={{fontSize:"9pt",fontWeight:500,color:"rgb(0,110,220)",textTransform:"uppercase",textAlign:"center"}}>{h}</div>)}
              </div>
              {/* SATIRLAR */}
              <div style={{maxHeight:"400px",overflow:"auto"}}>
                {gerKalemler.length===0
                  ?<div style={{padding:"24px",textAlign:"center",color:T.t3,fontSize:"12px"}}>Bağlı alış faturası yok.</div>
                  :gerKalemler.map((k,idx)=>{
                    const m=parseFloat(k.miktar||0);
                    const bf=parseFloat(k.netFiyat||0);
                    const kdvO=parseInt(k.kdvOrani||"20");
                    const kdvHar=m*bf;
                    const kdvTut=kdvHar*(kdvO/100);
                    const top=kdvHar+kdvTut;
                    const rowBg=idx%2===0?"#fff":"#fafafa";
                    return <div key={k.id} style={{display:"grid",gridTemplateColumns:gerCols,gap:"4px",padding:"3px 8px",alignItems:"center",borderBottom:idx<gerKalemler.length-1?`1px solid ${T.border}`:"none",background:rowBg}}>
                      <div style={{fontSize:"9pt",color:"rgb(0,110,220)",textAlign:"center",height:"30px",lineHeight:"30px"}}>{k.faturaTarihi?fmtDate(k.faturaTarihi):"—"}</div>
                      <div style={{fontSize:"9pt",color:"rgb(0,110,220)",textAlign:"center",height:"30px",lineHeight:"30px"}}>{k.afNo||"—"}</div>
                      <div style={{fontSize:"9pt",color:"rgb(0,110,220)",textAlign:"left",height:"30px",lineHeight:"30px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",padding:"0 4px"}} title={k.firmaAd||""}>{k.firmaAd||"—"}</div>
                      <div style={{fontSize:"10pt",color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,240,250)"}}>{m>0?m.toLocaleString("tr-TR"):"—"}</div>
                      <div style={{fontSize:"10pt",fontWeight:600,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,240,250)"}}>{bf>0?bf.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                      <div style={{fontSize:"10pt",color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,240,250)"}}>{kdvHar>0?kdvHar.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                      <div style={{fontSize:"10pt",color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,240,250)"}}>{kdvTut>0?kdvTut.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                      <div style={{fontSize:"11.5pt",fontWeight:600,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",border:`1px solid ${T.bDark}`,borderRadius:T.r,padding:"0 4px",boxSizing:"border-box",background:"rgb(230,240,250)"}}>{top>0?top.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    </div>;
                  })
                }
              </div>
              {/* TOPLAMLAR */}
              {gerKalemler.length>0&&<div style={{display:"grid",gridTemplateColumns:gerCols,gap:"4px",padding:"6px 8px",background:"#f0f5ff",borderTop:`2px solid ${T.primary}`}}>
                <div></div>
                <div></div>
                <div style={{fontSize:"12pt",fontWeight:700,color:T.text,textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>TOPLAM</div>
                <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gerMikTop>0?gerMikTop.toLocaleString("tr-TR"):"—"}</div>
                <div></div>
                <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gerKdvHTop>0?gerKdvHTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{gerKdvTop>0?gerKdvTop.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                <div style={{fontSize:"12pt",fontWeight:700,color:"rgb(0,110,220)",textAlign:"right",height:"30px",lineHeight:"30px",padding:"0 4px",boxSizing:"border-box"}}>{(gerKdvHTop+gerKdvTop)>0?(gerKdvHTop+gerKdvTop).toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
              </div>}
            </div>;
          })()}
          </div>;
...
const MaliyetPage=({projeler,setProjeler,malzemeler,faturalar=[],siparisler=[],firmalar=[],butceKalemleri:butceKalemleriGlobal=[],saveButceKalemi:saveBKProp,delButceKalemi:delBKProp,bulkSaveButceKalemleri:bulkSaveBKProp})=>{
  ...
  // Fiyatlandırma state
  const[fiyatForm,setFiyatForm]=useState({ongorulenM2KdvHaric:"",ongorulenM2KdvDahil:"",karMarjiYuzde:"",guncelKarMarjiYuzde:"",varsayilanKdvOrani:"20"});
  ...
  useEffect(()=>{
    if(selProje){
      setFiyatForm({
        ...
        guncelKarMarjiYuzde:selProje.guncelKarMarjiYuzde||"",
  ...
  const fiyatKaydet=async()=>{
    ...
    try{
      await sbPatch('projeler',selProjeId,{
        ...
        guncel_kar_marji_yuzde:fiyatForm.guncelKarMarjiYuzde!==""&&fiyatForm.guncelKarMarjiYuzde!=null?Number(fiyatForm.guncelKarMarjiYuzde):null,
  ...
  const blokFiyatHaricChange=(blokId,v)=>{
    ...
    const kdv=parseFloat(fiyatForm.varsayilanKdvOrani||"20");
    const haricN=parseFloat(cleaned);
    const dahilStr=!isNaN(haricN)?String(Math.round(haricN*(1+kdv/100))):"";
    setBlokFiyatForm(p=>({...p,[blokId]:{...(p[blokId]||{}),ongorulenM2KdvHaric:cleaned,ongorulenM2KdvDahil:dahilStr}}));
  };
  const blokFiyatDahilChange=(blokId,v)=>{
    const cleaned=v.replace(/[^0-9]/g,"");
    const kdv=parseFloat(fiyatForm.varsayilanKdvOrani||"20");
    const dahilN=parseFloat(cleaned);
    const haricStr=!isNaN(dahilN)?String(Math.round(dahilN/(1+kdv/100))):"";
    setBlokFiyatForm(p=>({...p,[blokId]:{...(p[blokId]||{}),ongorulenM2KdvHaric:haricStr,ongorulenM2KdvDahil:cleaned}}));
  };
  const setBlokFlag=(blokId,alan,v)=>setBlokFiyatForm(p=>({...p,[blokId]:{...(p[blokId]||{}),[alan]:v}}));
  ...
  const setDaireFlag=(boId,alan,v)=>setDaireFiyatForm(p=>({...p,[boId]:{...(p[boId]||{}),[alan]:v}}));
  const fmtInt=(v)=>{const n=parseFloat(v||0);return n>0?n.toLocaleString("tr-TR",{maximumFractionDigits:0}):"";};
  // Liste Fiyatı tek input (KDV Dahil, pazarlık payı eklenmiş son müşteri fiyatı)
  const daireListeChange=(boId,v)=>{
    const cleaned=v.replace(/[^0-9]/g,"");
    setDaireFiyatForm(p=>({...p,[boId]:{...(p[boId]||{}),listeFiyatiKdvDahil:cleaned}}));
  };
  ...
  const ozet=useMemo(()=>{
    ...
    // GÜNCEL: aynı mantık ama guncelMiktar × guncelBirimFiyat kullanır (boşsa planlanan fallback)
    const kalemKdvTutarGuncel=(k)=>{
      const plan=k.planlananSatirlari||[];
      let kdvHaric=0,kdv=0;
      if(plan.length>0){
        plan.forEach(s=>{
          const mG=parseFloat(s.guncelMiktar||s.miktar||0);
          const bfG=parseFloat(s.guncelBirimFiyat||s.birimFiyat||0);
          const kh=mG*bfG;
          kdvHaric+=kh;
          kdv+=kh*(parseInt(s.kdvOrani||0)/100);
        });
    ...
    const kalemTutar={};
    const kalemTutarGuncel={};
    butceKalemleri.forEach(k=>{kalemTutar[k.id]=kalemKdvTutar(k);kalemTutarGuncel[k.id]=kalemKdvTutarGuncel(k);});
    ...
    const guncelTop=butceKalemleri.reduce((s,k)=>s+kalemTutarGuncel[k.id].kdvHaric,0);
    const guncelTopKdvDahil=butceKalemleri.reduce((s,k)=>s+kalemTutarGuncel[k.id].kdvDahil,0);
    const faturaGerceklesenTop=gerceklesen.reduce((s,k)=>s+hareketKdvTutar(k).kdvHaric,0);
    const faturaGerceklesenTopKdvDahil=gerceklesen.reduce((s,k)=>s+hareketKdvTutar(k).kdvDahil,0);
    ...
    // MANUEL GERÇEKLEŞME — kapalı satırlarda plan-fatura farkı
    let manuelGerceklesenTop=0, manuelGerceklesenTopKdvDahil=0;
    butceKalemleri.forEach(k=>{
      const mlz=malzemeler.find(m=>m.id===k.malzemeId);
      const varsayilanKdv=parseInt(mlz?.kdvOrani||"20");
      const satirlar=k.planlananSatirlari||[];
      if(satirlar.length===0){
        if(!k.tamamlandi) return;
        const kalemFaturalar=gerceklesen.filter(g=>String(g.butceKalemiId)===String(k.id));
        const faturaHaric=kalemFaturalar.reduce((t,x)=>t+(parseFloat(x.netFiyat)||0)*(parseFloat(x.miktar)||0),0);
        const faturaDahil=kalemFaturalar.reduce((t,x)=>{
          const tut=(parseFloat(x.netFiyat)||0)*(parseFloat(x.miktar)||0);
          const o=!isNaN(parseInt(x.kdvOrani))?parseInt(x.kdvOrani):varsayilanKdv;
          return t+tut*(1+o/100);
        },0);
        const planHaric=kalemTutar[k.id].kdvHaric;
        const planDahil=kalemTutar[k.id].kdvDahil;
        manuelGerceklesenTop+=Math.max(0, planHaric-faturaHaric);
        manuelGerceklesenTopKdvDahil+=Math.max(0, planDahil-faturaDahil);
      } else {
        satirlar.forEach(s=>{
          if(!s.tamamlandi) return;
          const satirPlanHaric=(parseFloat(s.miktar)||0)*(parseFloat(s.birimFiyat)||0);
          const satirKdvOran=parseInt(s.kdvOrani||varsayilanKdv);
          const satirPlanDahil=satirPlanHaric*(1+satirKdvOran/100);
          const satirFaturalar=gerceklesen.filter(g=>String(g.butceKalemiSatirId)===String(s.id));
          const satirFaturaHaric=satirFaturalar.reduce((t,x)=>t+(parseFloat(x.netFiyat)||0)*(parseFloat(x.miktar)||0),0);
          const satirFaturaDahil=satirFaturalar.reduce((t,x)=>{
            const tut=(parseFloat(x.netFiyat)||0)*(parseFloat(x.miktar)||0);
            const o=!isNaN(parseInt(x.kdvOrani))?parseInt(x.kdvOrani):satirKdvOran;
            return t+tut*(1+o/100);
          },0);
          manuelGerceklesenTop+=Math.max(0, satirPlanHaric-satirFaturaHaric);
          manuelGerceklesenTopKdvDahil+=Math.max(0, satirPlanDahil-satirFaturaDahil);
        });
      }
    });
    const gerceklesenTop=faturaGerceklesenTop+manuelGerceklesenTop;
    const gerceklesenTopKdvDahil=faturaGerceklesenTopKdvDahil+manuelGerceklesenTopKdvDahil;
    ...
    // Blok bazlı maliyet hesabı (hem KDV hariç hem dahil) — PLANLANAN ve GÜNCEL paralel
    const blokMaliyet={};
    const blokMaliyetKdvDahil={};
    const gun_blokMaliyet={};
    const gun_blokMaliyetKdvDahil={};
    (selProje?.bloklar||[]).forEach(bl=>{blokMaliyet[bl.ad]=0;blokMaliyetKdvDahil[bl.ad]=0;gun_blokMaliyet[bl.ad]=0;gun_blokMaliyetKdvDahil[bl.ad]=0;});
    ...
    butceKalemleri.forEach(k=>{
      ...
      const tutarG=kalemTutarGuncel[k.id].kdvHaric;
      const tutarDahilG=kalemTutarGuncel[k.id].kdvDahil;
      const bloklar=k.bloklar||[];
      if(bloklar.length===0){/* atanmamış */}
      else if(bloklar.length===1){
        if(blokMaliyet[bloklar[0]]!==undefined){blokMaliyet[bloklar[0]]+=tutar;blokMaliyetKdvDahil[bloklar[0]]+=tutarDahil;gun_blokMaliyet[bloklar[0]]+=tutarG;gun_blokMaliyetKdvDahil[bloklar[0]]+=tutarDahilG;}
        aktifBloklar.add(bloklar[0]);
      }
      else{
        const topBlokM2=bloklar.reduce((s,b)=>s+(blokM2Harita[b]||0),0);
        if(topBlokM2>0){bloklar.forEach(b=>{const oran=(blokM2Harita[b]||0)/topBlokM2;if(blokMaliyet[b]!==undefined){blokMaliyet[b]+=tutar*oran;blokMaliyetKdvDahil[b]+=tutarDahil*oran;gun_blokMaliyet[b]+=tutarG*oran;gun_blokMaliyetKdvDahil[b]+=tutarDahilG*oran;}aktifBloklar.add(b);});}
        ortakToplam+=tutar;
    ...
    // Maliyet hesabında manuel girilen oran öncelikli; boşsa Müteahhit Payı fallback
    const muteahhitOran=parseFloat(selProje?.maliyetMuteahhitPay||selProje?.muteahhitPay||0)/100;
    ...
    // GÜNCEL versiyonlar
    const gun_m2Maliyet=brutM2Toplam>0?guncelTop/brutM2Toplam:0;
    const gun_m2MaliyetKdvDahil=brutM2Toplam>0?guncelTopKdvDahil/brutM2Toplam:0;
    const gun_m2MaliyetNet=muteahhitM2>0?guncelTop/muteahhitM2:0;
    const gun_m2MaliyetNetKdvDahil=muteahhitM2>0?guncelTopKdvDahil/muteahhitM2:0;
    ...
    return{planlananTop,planlananTopKdvDahil,guncelTop,guncelTopKdvDahil,gerceklesenTop,gerceklesenTopKdvDahil,faturaGerceklesenTop,faturaGerceklesenTopKdvDahil,manuelGerceklesenTop,manuelGerceklesenTopKdvDahil,taahhutTop,taahhutTopKdvDahil,kalanButce:planlananTop-taahhutTop-gerceklesenTop,kalanButceKdvDahil:planlananTopKdvDahil-taahhutTopKdvDahil-gerceklesenTopKdvDahil,brutM2,ortakM2,toplamM2,m2Maliyet,m2MaliyetKdvDahil,m2MaliyetNet,m2MaliyetNetKdvDahil,gun_m2Maliyet,gun_m2MaliyetKdvDahil,gun_m2MaliyetNet,gun_m2MaliyetNetKdvDahil,muteahhitM2,muteahhitOran,blokMaliyet,blokMaliyetKdvDahil,gun_blokMaliyet,gun_blokMaliyetKdvDahil,ortakToplam,ortakToplamKdvDahil,blokEfektifM2,kategoriDagilim,blokKategoriDagilim};
    ...
    {!selProje
      ...
      :<div>
        ...
        {aktifSekme==="dashboard"&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr) 1.3fr",gap:"12px",marginBottom:"16px"}}>
            {[
              ...
              {label:"Gerçekleşen",value:ozet.gerceklesenTop,valueDahil:ozet.gerceklesenTopKdvDahil,color:"#1677ff",tabular:{
                faturaHaric:ozet.faturaGerceklesenTop,faturaDahil:ozet.faturaGerceklesenTopKdvDahil,
                manuelHaric:ozet.manuelGerceklesenTop,manuelDahil:ozet.manuelGerceklesenTopKdvDahil
              }},
              {label:"Kalan Bütçe",value:ozet.kalanButce,valueDahil:ozet.kalanButceKdvDahil,color:ozet.kalanButce>=0?"#52c41a":"#ff4d4f"},
            ].map((c,i)=>{
              const fmt=(v)=>v?v.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—";
              return <div key={i} style={{padding:"16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff"}}>
                <div style={{fontSize:"12px",color:T.t3,marginBottom:"8px",textTransform:"uppercase"}}>{c.label}</div>
                {c.tabular?<>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"8px"}}>
                    <span style={{fontSize:"10px",color:T.t3,fontWeight:600}}>TOPLAM</span>
                    <span style={{fontSize:"20px",fontWeight:700,color:c.color}}>{fmt(c.value)}</span>
                  </div>
                  <div style={{borderTop:`1px dashed ${T.border}`,paddingTop:"6px",display:"flex",flexDirection:"column",gap:"3px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                      <span style={{fontSize:"10px",color:T.t3,fontWeight:500}}>Fatura ile</span>
                      <span style={{fontSize:"12px",fontWeight:600,color:c.color,opacity:0.85}}>{fmt(c.tabular.faturaHaric)}</span>
                    </div>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                      <span style={{fontSize:"10px",color:T.t3,fontWeight:500}}>Manuel kapatma</span>
                      <span style={{fontSize:"12px",fontWeight:600,color:c.color,opacity:0.85}}>{fmt(c.tabular.manuelHaric)}</span>
                    </div>
                  </div>
                </>:
                  <div style={{fontSize:"22px",fontWeight:700,color:c.color}}>{fmt(c.value)}</div>
                }
              </div>;
            })}
            {/* m² MALİYET KARTI */}
            <div style={{padding:"16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff"}}>
              ...
              <div style={{fontSize:"20px",fontWeight:700,color:"#722ed1"}}>{ozet.m2Maliyet>0?ozet.m2Maliyet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺/m²":"—"}</div>
              <div style={{fontSize:"11px",color:T.t3,marginTop:"4px"}}>Brüt: {ozet.brutM2.toLocaleString("tr-TR",{maximumFractionDigits:0})} m²</div>
              {ozet.muteahhitOran>0&&<>
                <div style={{borderTop:`1px solid ${T.border}`,marginTop:"8px",paddingTop:"8px"}}>
                  ...
                  <div style={{fontSize:"18px",fontWeight:700,color:"#ff4d4f"}}>{ozet.m2MaliyetNet>0?ozet.m2MaliyetNet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺/m²":"—"}</div>
                  <div style={{fontSize:"11px",color:T.t3,marginTop:"2px"}}>Müteahhit: {ozet.muteahhitM2.toLocaleString("tr-TR",{maximumFractionDigits:0})} m² (%{(ozet.muteahhitOran*100).toFixed(0)})</div>
                </div>
              </>}
            </div>
          </div>
          ...
          {(selProje?.bloklar||[]).length>0&&<div style={{display:"grid",gridTemplateColumns:`repeat(${(selProje.bloklar||[]).length+(ozet.ortakToplam>0?1:0)},1fr)`,gap:"12px",marginBottom:"16px"}}>
            {(selProje.bloklar||[]).map(bl=>{
              ...
              return <div key={bl.id} style={{padding:"12px 16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff"}}>
                ...
                <div style={{fontSize:"18px",fontWeight:700,color:"#1677ff",marginBottom:"6px"}}>{blMaliyet>0?blMaliyet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</div>
                <div style={{fontSize:"12px",color:T.t2}}>Brüt m²: <strong>{blBrutM2>0?blBrutM2.toLocaleString("tr-TR",{maximumFractionDigits:0}):0} m²</strong></div>
                <div style={{fontSize:"11px",color:"#722ed1",marginTop:"4px",fontWeight:600}}>Brüt m² Maliyet</div>
                <div style={{fontSize:"14px",fontWeight:700,color:"#722ed1",marginBottom:"4px"}}>{blM2Maliyet>0?blM2Maliyet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺/m²":"—"}</div>
                {ozet.muteahhitOran>0&&<>
                  <div style={{fontSize:"11px",color:"#ff4d4f",marginTop:"2px",fontWeight:600}}>Net m² Maliyet</div>
                  <div style={{fontSize:"14px",fontWeight:700,color:"#ff4d4f"}}>{blM2MaliyetNet>0?blM2MaliyetNet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺/m²":"—"}</div>
                </>}
              </div>;
            })}
            {ozet.ortakToplam>0&&<div style={{padding:"12px 16px",borderRadius:"8px",border:`1px solid #ffe58f`,background:"#fffbe6"}}>
              ...
              <div style={{fontSize:"18px",fontWeight:700,color:"#d48806"}}>{ozet.ortakToplam.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺"}</div>
          ...
          {Object.keys(ozet.kategoriDagilim||{}).length>0&&<div style={{border:`1px solid ${T.border}`,borderRadius:"8px",overflow:"hidden",marginBottom:"16px"}}>
            ...
            <div style={{padding:"4px 0"}}>
              {Object.entries(ozet.kategoriDagilim).sort((a,b)=>b[1].tutar-a[1].tutar).map(([kat,v],i)=><div key={kat} style={{display:"grid",gridTemplateColumns:"200px 120px 60px 1fr",gap:"12px",padding:"6px 16px",alignItems:"center",background:i%2===0?"#fff":"#fafafa"}}>
                ...
                <div style={{fontSize:"13px",fontWeight:600,color:T.text,textAlign:"right"}}>{v.tutar.toLocaleString("tr-TR",{maximumFractionDigits:0})} ₺</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"200px 120px 60px 1fr",gap:"12px",padding:"6px 16px",alignItems:"center",borderTop:`1px solid ${T.border}`,background:"#fafafa"}}>
                ...
                <div style={{fontSize:"13px",fontWeight:700,color:T.text,textAlign:"right"}}>{ozet.planlananTop.toLocaleString("tr-TR",{maximumFractionDigits:0})} ₺</div>
              ...
              {(()=>{
                ...
                return <>
                  ...
                  <div style={{flex:"0 0 auto",display:"grid",gridTemplateColumns:"30px 120px 0.7fr 100px 80px 60px 70px 80px 100px 80px 100px 90px 90px 90px 35px 25px",padding:"9px 12px",gap:"8px",alignItems:"center",background:"#384248",borderTop:`2px solid ${T.border}`}}>
                    ...
                    <div></div>
                    <div style={{fontSize:"15px",fontWeight:700,color:"#fff",textAlign:"right"}}>{topMiktar>0?topMiktar.toLocaleString("tr-TR",{maximumFractionDigits:2}):"—"}</div>
                    <div></div>
                    ...
                    {(()=>{
                      // Filtreli kalem ID'lerine bağlı taahhut/gerceklesen — filtre uygulanan kalemlerin toplamı
                      const filtreliIds=new Set(filtrelenmis.map(k=>k.id));
                      const tTop=taahhut.filter(t=>filtreliIds.has(t.butceKalemiId)).reduce((s,t)=>s+(parseFloat(t.netFiyat||0)*parseFloat(t.miktar||0)),0);
                      const gTop=gerceklesen.filter(g=>filtreliIds.has(g.butceKalemiId)).reduce((s,g)=>s+(parseFloat(g.netFiyat||0)*parseFloat(g.miktar||0)),0);
        ...
        {aktifSekme==="fiyatlandirma"&&<div>
          ...
          :<>
            {/* PROJE ÖZET KARTI — PLANLANAN | GÜNCEL yan yana */}
            <div style={{padding:"16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff",marginBottom:"16px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px",marginBottom:"14px"}}>
                {[
                  {kod:"plan",baslik:"PLANLANAN",renkBaslik:"rgb(255,128,64)",renkBaslikBg:"rgb(255,245,235)",brut:ozet.m2Maliyet,net:ozet.m2MaliyetNet,kar:fiyatForm.karMarjiYuzde,karKey:"karMarjiYuzde"},
                  {kod:"guncel",baslik:"GÜNCEL HESAPLAR",renkBaslik:"rgb(0,128,128)",renkBaslikBg:"rgb(230,245,245)",brut:ozet.gun_m2Maliyet,net:ozet.gun_m2MaliyetNet,kar:fiyatForm.guncelKarMarjiYuzde,karKey:"guncelKarMarjiYuzde"}
                ].map(sec=>{
                  const oran=parseFloat(selProje?.maliyetMuteahhitPay||selProje?.muteahhitPay||0);
                  const karNum=parseFloat(sec.kar)||0;
                  let toplamHaric=0,toplamM2=0;
                  (selProje.bloklar||[]).forEach(bl=>{
                    const bf=blokFiyatForm[bl.id];
                    const ongH=parseFloat(bf?.ongorulenM2KdvHaric);
                    if(isNaN(ongH)||ongH<=0)return;
                    const m2=blokM2Harita[bl.ad]||0;
                    if(m2<=0)return;
                    toplamHaric+=ongH*(1+karNum/100)*m2;
                    toplamM2+=m2;
                  });
                  const ortSatis=toplamM2>0?toplamHaric/toplamM2:0;
                  const satirStil={display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:`1px solid ${T.border}`};
                  return <div key={sec.kod} style={{padding:"0",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff",overflow:"hidden"}}>
                    <div style={{padding:"8px 12px",fontSize:"13px",fontWeight:700,color:sec.renkBaslik,background:sec.renkBaslikBg,letterSpacing:"0.5px",textAlign:"center"}}>{sec.baslik}</div>
                    <div style={{padding:"4px 12px"}}>
                      <div style={satirStil}>
                        <span style={{fontSize:"11px",color:T.t3,fontWeight:600,textTransform:"uppercase"}}>Hesaplanan Brüt m² (Sistem)</span>
                        <span style={{fontSize:"15px",fontWeight:700,color:"#722ed1"}}>{sec.brut>0?sec.brut.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                      </div>
                      <div style={satirStil}>
                        <span style={{fontSize:"11px",color:T.t3,fontWeight:600,textTransform:"uppercase"}}>Kat Karşılığı Oranı</span>
                        <span style={{fontSize:"15px",fontWeight:700,color:T.text}}>{oran>0?`%${oran}`:"—"}</span>
                      </div>
                      <div style={satirStil}>
                        <span style={{fontSize:"11px",color:T.t3,fontWeight:600,textTransform:"uppercase"}}>Hesaplanan Net m² (Müteahhit Payı)</span>
                        <span style={{fontSize:"15px",fontWeight:700,color:"#ff4d4f"}}>{sec.net>0?sec.net.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                      </div>
                      <div style={satirStil}>
                        <span style={{fontSize:"11px",color:T.t3,fontWeight:600,textTransform:"uppercase"}}>Kar Marjı (%)</span>
                        <input type="text" value={sec.kar} onChange={e=>setFiyatForm(p=>({...p,[sec.karKey]:e.target.value.replace(/[^0-9]/g,"")}))} placeholder="0" style={{width:"80px",padding:"4px 8px",border:`1px solid ${T.bDark}`,borderRadius:"4px",fontSize:"13px",textAlign:"right",fontWeight:700,color:sec.renkBaslik}}/>
                      </div>
                      <div style={{...satirStil,borderBottom:"none"}}>
                        <span style={{fontSize:"11px",color:T.t3,fontWeight:600,textTransform:"uppercase"}}>Ortalama m² Satış Fiyatı (Blok Ağırlıklı)</span>
                        <span style={{fontSize:"15px",fontWeight:700,color:"#52c41a"}}>{ortSatis>0?ortSatis.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                      </div>
                    </div>
                  </div>;
                })}
              ...
              {/* KDV Oranı + Save */}
              <div style={{display:"flex",alignItems:"end",gap:"16px",justifyContent:"space-between"}}>
                ...
                <button onClick={fiyatKaydet} disabled={!selProjeId} title={fiyatSaved?"Kaydedildi":"Kaydet"} style={{padding:"0",border:"none",background:"transparent",color:fiyatSaved?"#52c41a":"#8799a3",cursor:selProjeId?"pointer":"not-allowed",display:"flex",alignItems:"center",opacity:selProjeId?1:0.5,transition:"color .3s"}}><Save size={40}/></button>
              </div>
            </div>
              ...
              <div style={{padding:"16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(3,(selProje.bloklar||[]).length)},1fr)`,gap:"12px"}}>
                  {(selProje.bloklar||[]).map(bl=>{
                    const blBrutM2=blokM2Harita[bl.ad]||0;
                    const blMaliyet=ozet.blokMaliyet[bl.ad]||0;
                    const blMaliyetGuncel=ozet.gun_blokMaliyet[bl.ad]||0;
                    const blMutM2=ozet.muteahhitOran>0?blBrutM2*ozet.muteahhitOran:blBrutM2;
                    const blM2MaliyetNet=blMutM2>0?blMaliyet/blMutM2:0;
                    const blM2MaliyetNetGuncel=blMutM2>0?blMaliyetGuncel/blMutM2:0;
                    ...
                    // Sapma: öngörülen vs GÜNCEL hesaplanan net m² maliyet
                    const sapma=!isNaN(etkinHaric)&&blM2MaliyetNetGuncel>0?etkinHaric-blM2MaliyetNetGuncel:null;
                    const sapmaY=sapma!=null&&blM2MaliyetNetGuncel>0?(sapma/blM2MaliyetNetGuncel*100):null;
                    const sapmaCol=sapmaY==null?T.t3:Math.abs(sapmaY)<5?T.ok:Math.abs(sapmaY)<15?T.warn:T.err;
                    const planKar=parseFloat(fiyatForm.karMarjiYuzde)||0;
                    const gunKar=parseFloat(fiyatForm.guncelKarMarjiYuzde)||0;
                    const planSatis=!isNaN(etkinHaric)&&etkinHaric>0?etkinHaric*(1+planKar/100):0;
                    const gunSatis=!isNaN(etkinHaric)&&etkinHaric>0?etkinHaric*(1+gunKar/100):0;
                    return<div key={bl.id} style={{padding:"12px",borderRadius:"6px",border:`1px solid ${T.border}`,background:"#fafafa"}}>
                      ...
                      <div style={{fontSize:"10px",color:T.t3,textTransform:"uppercase",fontWeight:600,marginBottom:"4px"}}>Hesaplanan Net m² (Planlanan / Güncel)</div>
                      <div style={{fontSize:"14px",fontWeight:700,marginBottom:"10px"}}>
                        <span style={{color:"rgb(255,128,64)"}}>{blM2MaliyetNet>0?blM2MaliyetNet.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                        <span style={{color:T.t3,fontWeight:400,margin:"0 6px"}}>/</span>
                        <span style={{color:"rgb(0,128,128)"}}>{blM2MaliyetNetGuncel>0?blM2MaliyetNetGuncel.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                      </div>
                      ...
                      {!isNaN(etkinHaric)&&etkinHaric>0&&<div style={{marginTop:"8px",marginBottom:"8px",borderRadius:"6px",overflow:"hidden",border:`1px solid ${T.border}`}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:"rgb(255,245,235)"}}>
                          <span style={{fontSize:"10px",color:"rgb(255,128,64)",fontWeight:700,textTransform:"uppercase"}}>Planlanan (%{planKar})</span>
                          <span style={{fontSize:"14px",fontWeight:700,color:"rgb(255,128,64)"}}>{planSatis>0?planSatis.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                        </div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:"rgb(230,245,245)",borderTop:`1px solid ${T.border}`}}>
                          <span style={{fontSize:"10px",color:"rgb(0,128,128)",fontWeight:700,textTransform:"uppercase"}}>Güncel (%{gunKar})</span>
                          <span style={{fontSize:"14px",fontWeight:700,color:"rgb(0,128,128)"}}>{gunSatis>0?gunSatis.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺":"—"}</span>
                        </div>
                      </div>}
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"6px"}}>
                        <div style={{fontSize:"11px",color:sapmaCol,fontWeight:600}} title="Öngörülen m² vs Güncel Net m² maliyeti">
                          {sapma==null?"—":"Sapma (G): "+(sapma>=0?"+":"")+sapma.toLocaleString("tr-TR",{maximumFractionDigits:0})+" ₺ (%"+(sapmaY>=0?"+":"")+sapmaY.toFixed(0)+")"}
                        </div>
              ...
              <div style={{padding:"16px",borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff"}}>
                ...
                {(()=>{
                  ...
                  muteahhitDaireler.forEach(bo=>{
                    ...
                    if(bo.durum==="satildi"){satilan++;gerceklesenTop+=(parseFloat(bo.satisBedeli)||0)+(parseFloat(bo.plus)||0);}
            ...
            {(()=>{
              ...
              // GÜNCEL kar marjı kullanılır; boşsa planlanan kar marjına fallback
              const karMarji=parseFloat(fiyatForm.guncelKarMarjiYuzde||fiyatForm.karMarjiYuzde||"0");
              const karKaynak=fiyatForm.guncelKarMarjiYuzde?"Güncel":"Planlanan";
              const satilanSayisi=muteahhitDaireler.filter(bo=>bo.durum==="satildi").length;
              return <div style={{borderRadius:"8px",border:`1px solid ${T.border}`,background:"#fff",marginBottom:"16px",overflow:"hidden"}}>
                {/* HEADER BAR */}
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",background:"#384248"}}>
                  <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
                    <span style={{fontSize:"16px",fontWeight:700,color:"#fff"}}>Müteahhit Daireleri — Maliyet & Fiyat Planlaması</span>
                    <span style={{fontSize:"13px",color:"#8799a3"}}>{muteahhitDaireler.length} daire{satilanSayisi>0?`, ${satilanSayisi} satıldı`:""}</span>
                  </div>
                  <button onClick={()=>{
                    const rows=[["Blok","No","Kat","Cephe","Oda","Brut m2","Ongorulen m2","Daire Maliyet","Kar %","Uretim S.F.","Kat Sfye","Cephe Sfye","Toplam Satis","KDV","KDV Dahil Satis","Liste Fiyati","Gerceklesen Satis","Durum"]];
                    let xBrut=0,xDM=0,xUSF=0,xKS=0,xCS=0,xHSF=0,xKD=0,xL=0,xG=0;
                    muteahhitDaireler.sort((a,b)=>(a.blok||"").localeCompare(b.blok||"")||(a.kat||0)-(b.kat||0)||(a.no||"").localeCompare(b.no||"")).forEach(bo=>{
                      const f=daireFiyatForm[bo.id]||{};
                      const blok=(selProje.bloklar||[]).find(bl=>bl.ad===bo.blok);
                      ...
                      xBrut+=brutM2;xDM+=daireMaliyet;xUSF+=uretimSF;xKS+=katSrf;xCS+=cepheSrf;xHSF+=hesapSF;xKD+=kdvDahilSatis;xL+=listeTl;xG+=gerceklesenSatis;
                      rows.push([bo.blok||"",bo.no||"",bo.kat||"",bo.cephe||"",bo.odaSayisi||"",brutM2||"",!isNaN(blokOng)?Math.round(blokOng):"",Math.round(daireMaliyet)||"",karMarji||0,Math.round(uretimSF)||"",Math.round(katSrf)||"",Math.round(cepheSrf)||"",Math.round(hesapSF)||"",autoKdv,Math.round(kdvDahilSatis)||"",Math.round(listeTl)||"",Math.round(gerceklesenSatis)||"",durumMeta.label]);
                    });
                    rows.push(["TOPLAM","","","","",Math.round(xBrut)||"","",Math.round(xDM)||"","",Math.round(xUSF)||"",Math.round(xKS)||"",Math.round(xCS)||"",Math.round(xHSF)||"","",Math.round(xKD)||"",Math.round(xL)||"",Math.round(xG)||"",""]);
                    const csv=rows.map(r=>r.map(c=>`"${c}"`).join(";")).join("\n");
                    const blob=new Blob(["﻿"+csv],{type:"text/csv;charset=utf-8;"});
                    const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`${selProje.ad||"proje"}_muteahhit_daireleri.csv`;a.click();
                  }} title="Excel'e Aktar" style={{padding:"0",border:"none",background:"transparent",cursor:"pointer",display:"flex",alignItems:"center"}}>
                    <img src={excelIcon} alt="Excel" style={{width:"35px",height:"35px"}}/>
                  </button>
                </div>
                {/* INFO STRIP */}
                <div style={{padding:"8px 16px",fontSize:"11px",color:T.t3,background:"#fafafa",borderBottom:`1px solid ${T.border}`}}>Proje kârlılık hesabının kalbidir. Satılan daireler dahil müteahhide ait tüm daireler listelenir. Liste Fiyatı müteahhitin pazarlık başlangıç rakamıdır (KDV Dahil, satıştan bağımsız, her zaman düzenlenebilir). Gerçekleşen Satış satış modülünden gelir — Satış Bedeli (KDV Hariç Fatura Matrahı) + PLUS toplamıdır.</div>
                <div style={{overflowX:"auto"}}>
                  <div style={{display:"grid",gridTemplateColumns:"60px 50px 40px 80px 50px 60px 80px 95px 50px 105px 85px 85px 105px 50px 110px 110px 115px 100px",gap:"4px",fontSize:"11px",fontWeight:600,color:T.t2,textTransform:"uppercase",padding:"8px 4px",borderBottom:`1px solid ${T.border}`,alignItems:"center",minWidth:"1438px",textAlign:"center",background:"#fafafa"}}>
                    <div>Blok</div><div>No</div><div>Kat</div><div>Cephe</div><div>Oda</div><div>Brüt m²</div><div>Öngörülen m²</div><div>Daire Maliyet</div><div>Kar %</div><div>Satış Fiyatı</div><div>Kat Şfye</div><div>Cephe Şfye</div><div>Toplam Satış</div><div>KDV</div><div>KDV Dahil Satış</div><div>Liste Fiyatı</div><div>Gerçekleşen Satış</div><div>Durum</div>
                  </div>
                  {(()=>{
                    let topBrutM2=0,topDaireMaliyet=0,topUretimSF=0,topKatSrf=0,topCepheSrf=0,topHesapSF=0,topKdvDahil=0,topListe=0,topGerceklesen=0;
                    const sorted=muteahhitDaireler.sort((a,b)=>(a.blok||"").localeCompare(b.blok||"")||(a.kat||0)-(b.kat||0)||(a.no||"").localeCompare(b.no||""));
                    const rows=sorted.map(bo=>{
                      const f=daireFiyatForm[bo.id]||{};
                      const saved=daireFiyatSaved[bo.id];
                      const blok=(selProje.bloklar||[]).find(bl=>bl.ad===bo.blok);
                      const blokOng=blok&&blok.ongorulenM2KdvHaric!==""?parseFloat(blok.ongorulenM2KdvHaric):NaN;
                      const etkinOng=blokOng;
                      const brutM2=parseFloat(bo.brutM2)||0;
                      ...
                      const satBed=parseFloat(bo.satisBedeli)||0;
                      const plusTl=parseFloat(bo.plus)||0;
                      const gerceklesenSatis=satBed+plusTl;
                      const listeTl=parseFloat(f.listeFiyatiKdvDahil)||0;
                      const rowBg=satildi?"#fafafa":"#fff";
                      topBrutM2+=brutM2;
                      topDaireMaliyet+=daireMaliyet;
                      topUretimSF+=uretimSF;
                      topKatSrf+=katSrf;
                      topCepheSrf+=cepheSrf;
                      topHesapSF+=hesapSF;
                      topKdvDahil+=kdvDahilSatis;
                      topListe+=listeTl;
                      topGerceklesen+=gerceklesenSatis;
                      return<div key={bo.id} style={{display:"grid",gridTemplateColumns:"60px 50px 40px 80px 50px 60px 80px 95px 50px 105px 85px 85px 105px 50px 110px 110px 115px 100px",gap:"4px",fontSize:"13px",padding:"5px 4px",borderBottom:`1px solid ${T.border}`,alignItems:"center",minWidth:"1438px",background:rowBg}}>
                        ...
                        <div style={{textAlign:"center",color:T.t2}}>{brutM2>0?brutM2.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{textAlign:"right",color:"#8c8c8c",fontWeight:600}} title="Blok bazlı öngörülen m² maliyet">{!isNaN(etkinOng)?etkinOng.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        ...
                        <input type="text" value={f._listeEdit?(f.listeFiyatiKdvDahil||""):fmtInt(f.listeFiyatiKdvDahil)} onChange={e=>daireListeChange(bo.id,e.target.value)} onFocus={()=>setDaireFlag(bo.id,"_listeEdit",true)} onBlur={()=>{setDaireFlag(bo.id,"_listeEdit",false);daireFiyatKaydet(bo.id);}} placeholder="—" title="Müteahhitin pazarlık başlangıç fiyatı (KDV Dahil) — satıştan bağımsız manuel giriş" style={{padding:"4px 6px",border:`1px solid ${saved?"#52c41a":"#d48806"}`,borderRadius:"3px",fontSize:"13px",fontWeight:700,color:"#d48806",boxSizing:"border-box",outline:"none",width:"100%",textAlign:"right",background:"#fffbe6",transition:"border-color .3s"}}/>
                        <div title={satildi?`Satış Bedeli (KDV Hariç): ${satBed.toLocaleString("tr-TR")} ₺ · PLUS: ${plusTl.toLocaleString("tr-TR")} ₺ → Gerçekleşen Net ${gerceklesenSatis.toLocaleString("tr-TR")} ₺`:"Daire henüz satılmadı"} style={{padding:"4px 6px",borderRadius:"3px",fontSize:"13px",fontWeight:700,color:gerceklesenSatis>0?"#0050b3":T.t3,textAlign:"right",background:gerceklesenSatis>0?"#e6f4ff":"#fafafa",border:`1px solid ${gerceklesenSatis>0?"#91caff":T.border}`,boxSizing:"border-box",width:"100%"}}>{gerceklesenSatis>0?gerceklesenSatis.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                    ...
                    return<>{rows}
                      {/* ALT TOPLAMLAR */}
                      <div style={{display:"grid",gridTemplateColumns:"60px 50px 40px 80px 50px 60px 80px 95px 50px 105px 85px 85px 105px 50px 110px 110px 115px 100px",gap:"4px",padding:"10px 4px",background:"#384248",alignItems:"center",minWidth:"1438px",borderTop:`2px solid ${T.border}`}}>
                        <div></div><div></div><div></div><div></div><div></div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#fff",textAlign:"center"}}>{topBrutM2>0?topBrutM2.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div></div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#d4b3ff",textAlign:"right"}}>{topDaireMaliyet>0?topDaireMaliyet.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div></div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#bae0ff",textAlign:"right"}}>{topUretimSF>0?topUretimSF.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#fff",textAlign:"right"}}>{topKatSrf>0?topKatSrf.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#fff",textAlign:"right"}}>{topCepheSrf>0?topCepheSrf.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#52c41a",textAlign:"right"}}>{topHesapSF>0?topHesapSF.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div></div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#95de64",textAlign:"right"}}>{topKdvDahil>0?topKdvDahil.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#ffd666",textAlign:"right"}}>{topListe>0?topListe.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div style={{fontSize:"14px",fontWeight:700,color:"#69b1ff",textAlign:"right"}}>{topGerceklesen>0?topGerceklesen.toLocaleString("tr-TR",{maximumFractionDigits:0}):"—"}</div>
                        <div></div>
                      </div>
                    </>;
                  })()}
...
const MusteriPickerModal=({firmalar,onSelect,onYeniMusteri,onClose,baslik="MÜŞTERİ SEÇ",pickerIslem="satildi",listeFiyatiOneri="",initialFirma=null,initialSatisBedeli="",initialPlus="",bolum=null,projeAd="",bolumler=[]})=>{
  ...
  const[bedelEdit,setBedelEdit]=useState(false);
  ...
  const firmaSeciminiOnayla=(firma)=>{
    ...
    setSatisBedeli("");
    setPlus("");
    setSunumTel((firma?.telefon||"").replace(/\D/g,"").slice(0,11));
    setMode("onay");
  };
  const listedenDoldur=()=>{
    // Liste Fiyatı KDV Dahil → KDV hariç matraha çevir, PLUS sıfır (tam fatura varsayımı)
    const temiz=String(listeFiyatiOneri||"").replace(/[^0-9]/g,"");
    const kdvDahil=parseFloat(temiz)||0;
    const matrah=kdvDahil>0?Math.round(kdvDahil/(1+kdvOran/100)):0;
    setSatisBedeli(matrah>0?String(matrah):"");
    setPlus("");
  };
  const onayla=()=>{
    ...
    if(pickerIslem==="satildi"){
      if(!satisBedeli||parseFloat(satisBedeli)<=0){alert("Satış bedeli zorunludur.");return;}
    }
    onSelect(secilenFirma,{satisBedeli:satisBedeli||"",plus:plus||"",kdvOrani:String(kdvOran),paraBirimi:"TL"});
    onClose();
  };
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1100,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"24px 20px"}}>
    <div style={{background:"#fff",borderRadius:T.rl,width:"900px",height:"auto",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{padding:"12px 20px",background:"#384248",display:"flex",alignItems:"center",gap:"16px",borderRadius:`${T.rl} ${T.rl} 0 0`}}>
        ...
        {mode==="list"?<button onClick={()=>{onClose();onYeniMusteri&&onYeniMusteri();}} title="Yeni Müşteri Ekle (Firmalar)" style={{padding:"0",border:"none",background:"transparent",color:"#8799a3",cursor:"pointer",display:"flex",alignItems:"center"}}><SquarePlus size={30}/></button>:<div style={{width:"24px"}}></div>}
      </div>
      ...
      {mode==="onay"&&secilenFirma&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            ...
            <div>
              <label style={{...lS,fontSize:"12px",fontWeight:600,marginBottom:"6px",color:T.text}}>
                {pickerIslem==="satildi"?"Satış Bedeli (KDV Hariç, Fatura Matrahı)":"Opsiyon Bedeli (KDV Hariç, TL)"}
              ...
              <div style={{display:"flex",gap:"8px",alignItems:"stretch"}}>
                <input type="text" value={bedelEdit?satisBedeli:fmtFiyat(satisBedeli)} onChange={e=>setSatisBedeli(e.target.value.replace(/[^0-9]/g,""))} onFocus={()=>setBedelEdit(true)} onBlur={()=>setBedelEdit(false)} placeholder={listeFiyatiOneri?`Liste KDV hariç ~${fmtFiyat(Math.round((parseFloat(listeFiyatiOneri)||0)/(1+kdvOran/100)))} ₺`:"0"} autoFocus style={{flex:1,padding:"10px 12px",border:`1px solid ${T.bDark}`,borderRadius:T.r,fontSize:"16px",fontWeight:700,color:"#1677ff",outline:"none",textAlign:"right",background:"#fff"}}/>
                {parseFloat(listeFiyatiOneri||0)>0&&<button onClick={listedenDoldur} title="Liste Fiyatı (KDV Dahil) → KDV'yi düşerek matraha çevir" style={{padding:"0 14px",borderRadius:T.r,border:`1px solid ${T.primary}`,background:"#fff",color:T.primary,fontSize:"12px",fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>Listeden Doldur</button>}
              </div>
            </div>
            </>}

            {pickerIslem==="satildi"&&<>
              {/* KDV — Satış Bedelinin KDV'si (otomatik, kilitli) */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>
                <div>
                  <label style={{...lS,fontSize:"11px",color:T.t3,marginBottom:"4px"}}>KDV Oranı (Daire Kuralı)</label>
                  <div title={bolum?.netM2?`Net ${bolum.netM2} m² → %${kdvOran} (yönetmelik: <150→%10, ≥150→%20)`:"Net m² bilgisi yok — varsayılan %10"} style={{padding:"8px 10px",borderRadius:T.r,background:"#fafafa",border:`1px solid ${T.border}`,fontSize:"14px",fontWeight:700,color:"#8c8c8c",textAlign:"center",cursor:"not-allowed"}}>%{kdvOran}</div>
                </div>
                <div>
                  <label style={{...lS,fontSize:"11px",color:T.t3,marginBottom:"4px"}}>Satış KDV'si (Hesap)</label>
                  <div style={{padding:"8px 10px",borderRadius:T.r,background:"#fafafa",border:`1px solid ${T.border}`,fontSize:"14px",fontWeight:700,color:"#8c8c8c",textAlign:"right"}}>{fmtFiyat((parseFloat(satisBedeli)||0)*kdvOran/100)||"0"} ₺</div>
                </div>
              </div>

              {/* PLUS */}
              <div>
                <label style={{...lS,fontSize:"12px",fontWeight:600,marginBottom:"6px",color:"#d48806"}}>
                  ➕ PLUS <span style={{color:T.t3,fontSize:"11px",fontWeight:400,marginLeft:"4px"}}>(opsiyonel)</span>
                </label>
                <input type="text" value={plusEdit?plus:fmtFiyat(plus)} onChange={e=>setPlus(e.target.value.replace(/[^0-9]/g,""))} onFocus={()=>setPlusEdit(true)} onBlur={()=>setPlusEdit(false)} placeholder="0" style={{width:"100%",padding:"10px 12px",border:`1px solid #d48806`,borderRadius:T.r,fontSize:"16px",fontWeight:700,color:"#d48806",outline:"none",textAlign:"right",background:"#fffbe6",boxSizing:"border-box"}}/>
              </div>

              {/* CANLI FORMÜL KARTI */}
              {(()=>{
                const bed=parseFloat(satisBedeli)||0;
                const pls=parseFloat(plus)||0;
                const kdv=bed*kdvOran/100;
                const net=bed+pls;
                const brut=bed+kdv+pls;
                return <div style={{padding:"12px 14px",borderRadius:T.r,background:"linear-gradient(135deg,#f6ffed 0%,#d9f7be 100%)",border:"1px solid #b7eb8f",marginTop:"2px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"6px"}}>
                    <span style={{fontSize:"13px",fontWeight:700,color:"#237804"}}>💰 Gerçekleşen Satış (KDV Hariç)</span>
                    <span style={{fontSize:"18px",fontWeight:700,color:"#237804"}}>{fmtFiyat(net)||"0"} ₺</span>
                  </div>
                  <div style={{fontSize:"11px",color:"#389e0d",borderTop:"1px dashed #b7eb8f",paddingTop:"6px"}}>
                    Satış Bedeli + PLUS = {fmtFiyat(bed)||"0"} + {fmtFiyat(pls)||"0"}
                  </div>
                  <div style={{fontSize:"11px",color:T.t2,display:"flex",justifyContent:"space-between",marginTop:"6px"}}>
                    <span>🧾 Fatura Toplamı (Bedel + KDV)</span>
                    <span style={{fontWeight:600}}>{fmtFiyat(bed+kdv)||"0"} ₺</span>
                  </div>
                  <div style={{fontSize:"11px",color:T.t2,display:"flex",justifyContent:"space-between",marginTop:"2px"}}>
                    <span>🏠 Alıcının Ödediği (Brüt)</span>
                    <span style={{fontWeight:600}}>{fmtFiyat(brut)||"0"} ₺</span>
                  </div>
                </div>;
              })()}
            </>}
        ...
        <div style={{padding:"12px 16px",borderTop:`1px solid ${T.border}`,background:"#fafafa",display:"flex",gap:"8px",justifyContent:"flex-end"}}>
          ...
          <button onClick={onayla} style={{padding:"8px 20px",borderRadius:T.r,border:"none",background:pickerIslem==="satildi"?"#1677ff":(pickerIslem==="sunum"?"#52c41a":"#fa8c16"),color:"#fff",fontSize:"13px",fontWeight:600,cursor:"pointer"}}>{pickerIslem==="satildi"?"Satışı Onayla":(pickerIslem==="sunum"?"📋 Sunumu Kaydet":"Opsiyonu Onayla")}</button>
        </div>
      </>}
    </div>
  </div>;
...
const SatisRaporPage=({projeler,currentUser})=>{
  ...
  const tumBolumler=useMemo(()=>{
    ...
    (projeler||[]).forEach(p=>{
      (p.bolumler||[]).forEach(b=>{
        if(b.sahiplik!=="Müteahhit")return;
        out.push({...b,projeId:p.id,projeAd:p.ad});
      });
    });
    // Sıralama: Proje → Blok → No (doğal numerik)
    const numKey=(no)=>{const m=String(no||"").match(/(\d+)/);return m?parseInt(m[1],10):0;};
    return out.sort((a,b)=>{
      const pc=(a.projeAd||"").localeCompare(b.projeAd||"","tr");
      if(pc!==0)return pc;
      const bc=(a.blok||"").localeCompare(b.blok||"","tr");
      if(bc!==0)return bc;
      return numKey(a.no)-numKey(b.no)||(a.no||"").localeCompare(b.no||"","tr");
    });
  },[projeler]);
  ...
  const durumOzet=useMemo(()=>{
    ...
    tumBolumler.forEach(b=>{if(o[b.durum]!==undefined)o[b.durum]++;});
    return o;
  },[tumBolumler]);

  // Görünür satırlar — durum filtresi + arama
  ...
  const topListe=gorunur.reduce((s,b)=>s+(parseFloat(b.listeFiyatiKdvDahil)||0),0);
  const fiyatsizSayi=gorunur.filter(b=>!(parseFloat(b.listeFiyatiKdvDahil)>0)).length;

  // Filtre kartı
  const Kart=({id,label,sayi,toplam,renk,bg})=>{
    const aktif=durumFiltre===id;
    return <button onClick={()=>setDurumFiltre(id)} style={{
      display:"flex",alignItems:"center",gap:"10px",padding:"0 16px",height:"40px",
      borderRadius:"6px",border:`1px solid ${aktif?renk:"transparent"}`,
      background:aktif?bg:"#384248",cursor:"pointer",
      transition:"all .15s"
    }}>
  ...
  return <div>
    ...
    <div style={{display:"flex",alignItems:"flex-start",gap:"16px",flexWrap:"wrap",marginBottom:"12px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",justifyContent:"space-between",alignItems:"flex-end",flexShrink:0,alignSelf:"stretch",gap:"8px"}}>
        <select style={{...iS,width:"240px",cursor:"pointer"}} value={projeFiltre} onChange={e=>setProjeFiltre(e.target.value)} onFocus={foc} onBlur={blr}>
    ...
    {fiyatsizSayi>0&&<div style={{padding:"8px 14px",borderRadius:T.r,background:"#fff7e6",border:`1px solid #fa8c1644`,color:"#d4880f",fontSize:"12px",marginBottom:"10px",display:"flex",alignItems:"center",gap:"8px"}}>
      ⚠️ Görünen listede <strong>{fiyatsizSayi}</strong> bölümün liste fiyatı henüz girilmemiş — toplama dahil edilmedi (Maliyet → Fiyatlandırma sekmesinden girilir).
    </div>}
    ...
    <div style={{background:"#fff",border:`1px solid ${T.border}`,borderRadius:T.rl,overflow:"hidden"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:cols,background:"#fafafa",borderBottom:`1px solid ${T.border}`,padding:"10px 14px",gap:"10px"}}>
        {["Proje Adı","Tip","Blok","No","Kat","Brüt m²","Oda","Cephe","Durum","Liste Fiyatı"].map((h,i)=>
          <div key={i} style={{fontSize:"12px",fontWeight:600,color:T.t2,textTransform:"uppercase",letterSpacing:"0.3px",textAlign:i>=5&&i<=6?"right":(i===9?"right":"left")}}>{h}</div>
        )}
      </div>
      {/* Satırlar */}
      {gorunur.length===0
        ?<div style={{padding:"40px",textAlign:"center",color:T.t3,fontSize:"14px"}}>Bu kritere uyan bölüm yok.</div>
        :gorunur.map((b,idx)=>{
          const ds=BOLUM_DURUMLARI.find(d=>d.id===b.durum)||{color:"#8c8c8c",bg:"#fafafa",label:b.durum||"—"};
          const lf=parseFloat(b.listeFiyatiKdvDahil)||0;
          return <div key={`${b.projeId}_${b.id}`} style={{display:"grid",gridTemplateColumns:cols,padding:"10px 14px",gap:"10px",alignItems:"center",borderBottom:idx<gorunur.length-1?`1px solid ${T.border}`:"none",background:idx%2===0?"#fff":"#fafafa"}}>
            <div style={{fontSize:"13px",color:T.text,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={b.projeAd}>{b.projeAd||"—"}</div>
            <div style={{fontSize:"13px",color:T.t2}}>{b.tipi||"—"}</div>
            <div style={{fontSize:"13px",color:T.text,fontWeight:600}}>{b.blok||"—"}</div>
            <div style={{fontSize:"13px",color:T.text,fontWeight:600}}>{b.no||"—"}</div>
            <div style={{fontSize:"13px",color:T.text}}>{b.kat||"—"}</div>
            <div style={{fontSize:"13px",color:T.text,textAlign:"right"}}>{b.brutM2?Number(b.brutM2).toLocaleString("tr-TR"):"—"}</div>
            <div style={{fontSize:"13px",color:T.t2,textAlign:"right"}}>{b.odaSayisi||"—"}</div>
            <div style={{fontSize:"13px",color:T.t2}}>{b.cephe||"—"}</div>
            <div><span style={{padding:"2px 8px",borderRadius:"4px",fontSize:"12px",color:ds.color,background:ds.bg,border:`1px solid ${ds.color}33`,whiteSpace:"nowrap"}}>{ds.label}</span></div>
            <div style={{fontSize:"13px",fontWeight:600,color:lf>0?T.text:T.t3,textAlign:"right"}}>{lf>0?lf.toLocaleString("tr-TR")+" ₺":"—"}</div>
          </div>;
        })
      }
      {/* TOPLAM SATIRI */}
      {gorunur.length>0&&<div style={{display:"grid",gridTemplateColumns:cols,padding:"10px 14px",gap:"10px",alignItems:"center",background:"#384248",borderTop:`2px solid ${T.border}`}}>
        <div style={{fontSize:"14px",fontWeight:700,color:"#fff"}}>Toplam ({gorunur.length} bölüm)</div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div style={{fontSize:"14px",fontWeight:700,color:"#fff",textAlign:"right"}}>{topBrut>0?topBrut.toLocaleString("tr-TR")+" m²":"—"}</div>
        <div></div>
        <div></div>
        <div></div>
...
const SatisSunumPage=({projeler,setProjeler,firmalar,saveProje,saveFirma,setPage,goToFirma,goToYeniFirma,gomulu=false,currentUser,sunumlar=[],saveSunum,delSunum,saveHatirlatma})=>{
  ...
  // Satıcı (gomulu/iPad): kuti parent'ın kalan alanını tam doldurur (cap yok, magic number yok)
  ...
  // Proje özetleri (durum dağılımı) — sadece Müteahhit'e ait daireler (satış modülü kapsamı)
  const projeOzet=(proje)=>{
    const b=(proje.bolumler||[]).filter(x=>x.sahiplik==="Müteahhit");
    const o={musait:0,opsiyonlu:0,satildi:0,kiralik:0};
    b.forEach(x=>{if(o[x.durum]!==undefined)o[x.durum]++;});
    return{...o,toplam:b.length};
  };

  // Blok özet (bu bloğa ait Müteahhit daireleri)
  const blokOzet=(blokAd)=>{
    const b=(selProje?.bolumler||[]).filter(x=>x.blok===blokAd&&x.sahiplik==="Müteahhit");
    const o={musait:0,opsiyonlu:0,satildi:0,kiralik:0};
    b.forEach(x=>{if(o[x.durum]!==undefined)o[x.durum]++;});
    return{...o,toplam:b.length};
  };
  ...
  const bolumGuncelle=async(bolumId,degisiklik)=>{
    ...
    if(degisiklik.satisBedeli!==undefined)bDb.satis_bedeli=degisiklik.satisBedeli!==""&&degisiklik.satisBedeli!=null?Number(degisiklik.satisBedeli):null;
    if(degisiklik.plus!==undefined)bDb.plus=degisiklik.plus!==""&&degisiklik.plus!=null?Number(degisiklik.plus):null;
    if(degisiklik.kdvOrani!==undefined)bDb.kdv_orani=degisiklik.kdvOrani||null;
  ...
  const musteriSec=async(firma,satisVerisi={})=>{
    ...
    if(satisVerisi.satisBedeli!==undefined&&satisVerisi.satisBedeli!==""){
      degisiklik.satisBedeli=parseFloat(satisVerisi.satisBedeli)||0;
    }
    if(satisVerisi.plus!==undefined){
      degisiklik.plus=parseFloat(satisVerisi.plus)||0;
    }
    if(satisVerisi.kdvOrani)degisiklik.kdvOrani=satisVerisi.kdvOrani;
  ...
  // Sunum kaydı — opsiyon öncesi görüşme. Daire durumuna DOKUNMAZ (müsait kalır).
  ...
  const musaitYap=async()=>{
    if(!selBolum||!confirm("Daire müsait durumuna döndürülsün mü? Müşteri bağlantısı ve satış bilgileri (Satış Bedeli, PLUS) kaldırılacak."))return;
    await bolumGuncelle(selBolum.id,{durum:"musait",aliciFirmaId:"",aliciFirmaAd:"",sozlesmeTarihi:"",satisBedeli:0,plus:0});
  };

  // Gerçekleşen satış fiyatını düzeltme (satış/opsiyondan sonra) — Satış Bedeli + PLUS ayrı ayrı
  const fiyatDuzelt=async()=>{
    if(!selBolum)return;
    const mevcutBed=parseFloat(selBolum.satisBedeli)||0;
    const mevcutPlus=parseFloat(selBolum.plus)||0;
    const yeniBed=prompt(`${selBolum.blok} Blok ${selBolum.no} — Satış Bedeli (KDV Hariç, Fatura Matrahı, TL):`,mevcutBed>0?String(mevcutBed):"");
    if(yeniBed===null)return;
    const temizBed=String(yeniBed).replace(/[^0-9]/g,"");
    if(!temizBed||parseFloat(temizBed)<=0){alert("Geçerli bir satış bedeli girin.");return;}
    const yeniPlus=prompt(`${selBolum.blok} Blok ${selBolum.no} — PLUS (TL, boş=0):`,mevcutPlus>0?String(mevcutPlus):"0");
    if(yeniPlus===null)return;
    const temizPlus=String(yeniPlus).replace(/[^0-9]/g,"");
    await bolumGuncelle(selBolum.id,{satisBedeli:parseFloat(temizBed),plus:parseFloat(temizPlus)||0});
  };
  ...
  const content=<div style={tamEkran?{position:"fixed",inset:0,background:"#fff",zIndex:900,padding:`${PAD}px`,overflow:"hidden",display:"flex",justifyContent:"center",alignItems:"stretch"}:(gomulu?{height:"100%",display:"flex",flexDirection:"column",overflow:"hidden"}:{})}>
    ...
    {detayBolum&&(()=>{
      const b=detayBolum;
      const sunumList=(sunumlar||[]).filter(s=>String(s.bolumId)===String(b.id)).sort((x,y)=>String(y.tarih).localeCompare(String(x.tarih)));
      const ds=durumRenk(b.durum);
      const opsiyonVar=(b.durum==="opsiyonlu"||b.durum==="satildi");
      return <div onClick={()=>setDetayBolum(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
        <div onClick={(e)=>e.stopPropagation()} style={{background:"#fff",borderRadius:T.rl,maxWidth:"520px",width:"100%",maxHeight:"80vh",overflowY:"auto",boxShadow:T.shM}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 20px",borderBottom:`1px solid ${T.border}`}}>
            <div>
              <div style={{fontSize:"12px",color:T.t3}}>{b.blok?`${b.blok} Blok • `:""}Daire {b.no||"—"}</div>
              <div style={{fontSize:"18px",fontWeight:700,color:T.text}}>Daire Detayı</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
              <span style={{padding:"3px 10px",borderRadius:"4px",background:ds.color,color:"#fff",fontSize:"12px",fontWeight:700,textTransform:"uppercase"}}>{ds.label}</span>
              <button onClick={()=>setDetayBolum(null)} style={{background:"none",border:"none",fontSize:"20px",color:T.t3,cursor:"pointer",lineHeight:1}}>✕</button>
            </div>
          </div>
          <div style={{padding:"16px 20px",display:"flex",flexDirection:"column",gap:"16px"}}>
            {opsiyonVar&&b.aliciFirmaAd&&<div style={{padding:"12px 14px",background:"#fff7e6",borderRadius:T.r,border:"1px solid #ffd591"}}>
              <div style={{color:"#d48806",fontWeight:600,fontSize:"13px",marginBottom:"4px"}}>👤 {b.durum==="satildi"?"Alıcı":"Opsiyon Sahibi"}</div>
              <div style={{color:T.text,fontSize:"15px",fontWeight:600}}>{b.aliciFirmaAd}</div>
              {b.sozlesmeTarihi&&<div style={{color:T.t3,fontSize:"12px",marginTop:"2px"}}>Tarih: {b.sozlesmeTarihi}</div>}
              {(()=>{const bed=(parseFloat(b.satisBedeli)||0)+(parseFloat(b.plus)||0);return bed>0?<div style={{color:T.t2,fontSize:"13px",marginTop:"4px",fontWeight:700}}>{bed.toLocaleString("tr-TR")} ₺</div>:null;})()}
            </div>}
            <div>
              <div style={{fontSize:"13px",fontWeight:700,color:T.text,marginBottom:"8px",display:"flex",alignItems:"center",gap:"6px"}}>📋 Sunum Geçmişi <span style={{fontSize:"12px",color:T.t3,fontWeight:500}}>({sunumList.length})</span></div>
              {sunumList.length===0
                ?<div style={{padding:"16px",textAlign:"center",color:T.t3,fontSize:"13px",background:"#fafafa",borderRadius:T.r,border:`1px solid ${T.border}`}}>Bu daireye sunum yapılmamış.</div>
                :<div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                  {sunumList.map(s=><div key={s.id} style={{padding:"10px 12px",background:"#fafafa",border:`1px solid ${T.border}`,borderRadius:T.r,fontSize:"13px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
                      <span style={{fontWeight:600,color:T.text}}>{s.firmaAd||"—"}</span>
                      <span style={{color:T.t3,fontSize:"12px"}}>{s.tarih||""}</span>
                    </div>
                    <div style={{color:T.t2,marginTop:"2px"}}>📞 {s.telefon||"—"}{s.saticiAd?` • ${s.saticiAd}`:""}</div>
                    {s.notlar&&<div style={{color:T.t3,marginTop:"3px",fontStyle:"italic",whiteSpace:"pre-wrap"}}>{s.notlar}</div>}
                    <button onClick={()=>delSunum&&delSunum(s.id)} style={{marginTop:"6px",padding:"3px 10px",borderRadius:"4px",border:`1px solid ${T.border}`,background:"#fff",color:T.t3,fontSize:"12px",cursor:"pointer"}}>Sil</button>
                  </div>)}
                </div>}
            </div>
          </div>
    ...
    {adim===2&&selProje&&(()=>{
      ...
      const kutStil=gomulu
        ?{height:"100%",width:"100%",maxWidth:`${SATICI_UI.maxW}px`,margin:"0 auto",position:"relative",flexShrink:0,minHeight:0}
        :{aspectRatio:"3/2",maxHeight:sliderH,width:"100%",maxWidth:`${SATICI_UI.maxW}px`,margin:"0 auto",position:"relative"};
      return <div style={{flex:"1 1 auto",minHeight:0,display:"flex",alignItems:gomulu?"stretch":"center",justifyContent:"center",width:"100%",overflow:"hidden"}}>
        <div style={kutStil}>
    ...
    {adim===4&&selProje&&selBlok&&<div style={{display:"grid",gridTemplateColumns:selBolum?(tamEkran?"1fr 460px":"1fr 400px"):"1fr",gap:tamEkran?"18px":"16px",flex:"1 1 auto",minHeight:0,overflow:"hidden",alignItems:"stretch",maxWidth:gomulu?`${SATICI_UI.daireEkranW}px`:undefined,marginLeft:gomulu?"auto":undefined,marginRight:gomulu?"auto":undefined,width:gomulu?"100%":undefined}}>
        ...
        {gorunurDaireler.length===0
          ...
          :<div style={{display:"grid",gridTemplateColumns:tamEkran?(selBolum?"repeat(5,1fr)":"repeat(6,1fr)"):"repeat(5,1fr)",gap:tamEkran?"14px":"12px",overflowY:"auto",overflowX:"hidden",alignContent:"flex-start",paddingRight:"4px"}}>
            {gorunurDaireler.map(b=>{
              ...
              const sunumSay=(sunumlar||[]).filter(s=>String(s.bolumId)===String(b.id)).length;
              return <div key={b.id} onClick={()=>setSelBolumId(b.id)} style={{background:satildi?"#f5f5f5":d.bg,border:`2px solid ${sec?d.color:(satildi?"#d9d9d9":d.color+"55")}`,borderRadius:T.rl,padding:tamEkran?"18px 16px":"14px",cursor:"pointer",transition:"all .15s",position:"relative",transform:sec?"scale(1.02)":"scale(1)",boxShadow:sec?T.shM:"none",opacity:satildi?0.55:1,filter:satildi?"grayscale(0.6)":"none",minHeight:tamEkran?"150px":"auto"}}
                ...
                {sunumSay>0&&<div onClick={(e)=>{e.stopPropagation();setDetayBolum(b);}} title="Sunum/opsiyon detayını gör" style={{position:"absolute",top:"8px",right:"8px",display:"flex",alignItems:"center",gap:"4px",padding:"5px 11px",borderRadius:"12px",background:"#e6f4ff",color:"#0958d9",fontSize:"13px",fontWeight:700,border:"1px solid #91caff",zIndex:3,cursor:"pointer"}}>📋 {sunumSay}</div>}
                ...
                <div onClick={(b.durum==="opsiyonlu"||b.durum==="satildi")?(e)=>{e.stopPropagation();setDetayBolum(b);}:undefined} title={(b.durum==="opsiyonlu"||b.durum==="satildi")?"Opsiyon/satış detayını gör":undefined} style={{fontSize:"11px",padding:"4px 10px",borderRadius:"4px",background:d.color,color:"#fff",fontWeight:700,textTransform:"uppercase",display:"inline-block",letterSpacing:"0.3px",cursor:(b.durum==="opsiyonlu"||b.durum==="satildi")?"pointer":"default"}}>{d.label}</div>
              </div>;
            })}
          </div>}
      </div>
      ...
      {selBolum&&<div style={{background:"#fff",borderRadius:T.rl,border:`1px solid ${T.border}`,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
        ...
        <div style={{padding:tamEkran?"20px":"16px",overflowY:"auto",flex:"1 1 auto",minHeight:0}}>
          {/* Başlık satırı — sol: blok+daire no, sağ: fiyat düz yazı (resmin dışında) */}
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:"12px",marginBottom:"14px"}}>
            <div style={{minWidth:0}}>
              ...
              <div style={{fontSize:tamEkran?"32px":"28px",fontWeight:700,color:T.text}}>Daire {selBolum.no||"—"}</div>
            </div>
            {(()=>{
              const listeDahil=parseFloat(selBolum.listeFiyatiKdvDahil)||0;
              const satFiyat=(parseFloat(selBolum.satisBedeli)||0)+(parseFloat(selBolum.plus)||0);
              const durumKapali=selBolum.durum==="satildi"||selBolum.durum==="opsiyonlu";
              let val=0,sub="";
              if(durumKapali){val=satFiyat;sub=selBolum.durum==="satildi"?"Gerçekleşen • KDV Hariç":"Opsiyon • KDV Hariç";}
              else if(listeDahil>0){val=listeDahil;const n=parseFloat(selBolum.netM2);sub=`Liste • KDV Dahil (%${!isNaN(n)&&n>=150?20:10})`;}
              if(val<=0)return null;
              return <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:tamEkran?"26px":"22px",fontWeight:700,color:T.text,lineHeight:1.1,whiteSpace:"nowrap"}}>{val.toLocaleString("tr-TR",{maximumFractionDigits:0})} ₺</div>
                <div style={{fontSize:"11px",color:T.t3,marginTop:"2px",whiteSpace:"nowrap"}}>{sub}</div>
              </div>;
            })()}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:tamEkran?"10px":"8px"}}>
            ...
            {selBolum.durum==="musait"&&<>
              ...
              <button onClick={()=>{setPickerIslem("satildi");setPickerAcik(true);}} style={{padding:tamEkran?"16px":"12px",borderRadius:T.r,border:"none",background:"#1677ff",color:"#fff",fontSize:tamEkran?"16px":"14px",fontWeight:700,cursor:"pointer",minHeight:tamEkran?"52px":"auto"}}>🔵 SATIŞ</button>
            </>}
...
export default function App(){
  ...
  const[firmaYeniAc,setFirmaYeniAc]=useState(0);
  const goToYeniFirma=()=>{setFirmaYeniAc(n=>n+1);setPage("firmalar");};
  ...
  /* ---- YETKİSİZ SAYFA REDIRECT ----
     Kullanıcı rolüne uygun olmayan bir page state'ine düşerse rol bazlı home'a yönlendir. */
  useEffect(() => {
    if(currentUser && !canSee(page, currentUser)) {
      setPage(HOME_PAGE_BY_ROL[currentUser.rol] || "dashboard");
    }
  }, [page, currentUser]);
  ...
  const loadAll = useCallback(async (ilkYukleme=false) => {
    ...
    try {
      ...
      setButceKalemleri((bkalDb||[]).map(bk => {
        const local = butceKalemiToLocal(bk);
        // butce_kalemi_satirlari ayrı tablodan satırları attach et (eski JSONB sütunu paralel duruyor — fallback olarak)
        const sqlSatirlar = (bksatDb||[]).filter(s => s.butce_kalemi_id === bk.id).map(s => {
          const sLocal = kalemSatirToLocal(s);
          // Her satıra revizyonlarını ekle
          sLocal.revizyonlar = (bksrDb||[]).filter(r => r.butce_kalemi_satir_id === s.id).map(satirRevToLocal);
          return sLocal;
        });
        if (sqlSatirlar.length > 0) local.planlananSatirlari = sqlSatirlar;
        return local;
      }));
    } catch(e) {
  ...
  useEffect(() => {
    ...
    const channel = sbClient.channel('realtime-operon')
      ...
      // butce_kalemi_satirlari — butceKalemleri[].planlananSatirlari (sessiz)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'butce_kalemi_satirlari' }, (p) => {
        if(isDel(p)) { const id=oldId(p); if(!id) return;
          setButceKalemleri(prev => prev.map(bk => ({...bk, planlananSatirlari: remove(bk.planlananSatirlari||[], id)})));
        } else {
          const local = kalemSatirToLocal(p.new);
          setButceKalemleri(prev => prev.map(bk => bk.id===local.butceKalemiId ? {...bk, planlananSatirlari: upsert(bk.planlananSatirlari||[], {...local, revizyonlar: (bk.planlananSatirlari||[]).find(s=>s.id===local.id)?.revizyonlar||[]})} : bk));
        }
      })
      // butce_kalemi_satir_revizyonlari — satır içindeki revizyonlar[] (sessiz)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'butce_kalemi_satir_revizyonlari' }, (p) => {
        if(isDel(p)) { const id=oldId(p); if(!id) return;
          setButceKalemleri(prev => prev.map(bk => ({...bk, planlananSatirlari: (bk.planlananSatirlari||[]).map(s => ({...s, revizyonlar: remove(s.revizyonlar||[], id)}))})));
        } else {
          const local = satirRevToLocal(p.new);
          setButceKalemleri(prev => prev.map(bk => ({...bk, planlananSatirlari: (bk.planlananSatirlari||[]).map(s => s.id===local.butceKalemiSatirId ? {...s, revizyonlar: upsert(s.revizyonlar||[], local)} : s)})));
        }
      })
      ...
      // sunumlar — top-level (opsiyon öncesi görüşme / lead kayıtları)
  ...
  useEffect(()=>{
    ...
    (async()=>{
      for(const bk of butceKalemleri) {
        ...
        // Alt satırsız kalem: aggregate kontrol
        if(altlar.length===0){
        ...
        // Alt satırları tek tek değerlendir
        let yeniSat=altlar.map(s=>s.tamamlandi?s:(altSatirEslesti(s, bk.id, faturalar)?{...s,tamamlandi:true}:s));
        let altlarHepsiKapali=yeniSat.every(s=>s.tamamlandi===true);
        let yeniAnaKapali=anaKapali;
        // Aggregate fallback (eski faturalarda butceKalemiSatirId yoksa): hepsini kapat
        if(!altlarHepsiKapali&&!anaKapali&&isBkAutoClosed(bk, faturalar)){
          yeniSat=yeniSat.map(s=>({...s,tamamlandi:true}));
          altlarHepsiKapali=true;
        }
        if(!anaKapali&&altlarHepsiKapali) yeniAnaKapali=true;
        // Ana zaten kapalıysa tüm altları senkronize et
        if(anaKapali&&!altlarHepsiKapali){
          yeniSat=yeniSat.map(s=>({...s,tamamlandi:true}));
          altlarHepsiKapali=true;
        }
        const satDegisti=JSON.stringify(yeniSat)!==JSON.stringify(altlar);
        const anaDegisti=yeniAnaKapali!==anaKapali;
        if(satDegisti||anaDegisti){
          const patchData={planlanan_satirlari:yeniSat};
          if(anaDegisti){patchData.tamamlandi=true; patchData.tamamlandi_kaynak='otomatik';}
          try {
            await sbPatch('butce_kalemleri', bk.id, patchData);
            setButceKalemleri(prev=>prev.map(b=>b.id===bk.id?{...b, planlananSatirlari:yeniSat, ...(anaDegisti?{tamamlandi:true,tamamlandiKaynak:'otomatik'}:{})}:b));
          } catch(e) { console.warn("Backfill kalem güncelleme hatası:", e.message); }
        }
      }
      // SP durum senkronizasyonu — açık SP'leri faturalara göre yeniden değerlendir
      for(const sp of siparisler) {
        if(!['taslak','onaylandi','kismen_teslim'].includes(sp.durum)) continue;
        const yeniDurum = computeSpDurumu(sp, faturalar);
        if(yeniDurum && yeniDurum!==sp.durum) {
          try {
            await sbPatch('satinalma_siparisleri', sp.id, {durum:yeniDurum});
            setSiparisler(prev=>prev.map(s=>s.id===sp.id?{...s,durum:yeniDurum}:s));
          } catch(e) { console.warn("Backfill SP durum güncelleme hatası:", e.message); }
        }
      }
    })();
  },[loading,butceKalemleri,faturalar,siparisler]);
  ...
  const saveFirma = async (form) => {
    ...
    try {
      ...
      // Satıcı yeni firma eklerken sahipliği otomatik kaydet (sahiplik filtresi açıldığında lazım)
      if(form._isNew && isSatici(currentUser)) {
        dbData.satici_id = currentUser.id;
        dbData.atama_tarihi = new Date().toISOString();
      }
      let firmaId;
      if(form._isNew) {
  ...
  const saveFatura = async (form) => {
    ...
    try {
      ...
      // Otomatik sipariş kapatma — form.spId + fatura kalemlerinin butceKalemiId'sinden ilgili SP'leri bul
      {
        const tumFaturalar = [...faturalar.filter(f=>f.id!==form.id), form];
        const ilgiliSpIds = new Set();
        if(form.spId) ilgiliSpIds.add(parseInt(form.spId));
        const bkIdsInForm = new Set((form.kalemler||[]).map(k=>k.butceKalemiId).filter(Boolean));
        if(bkIdsInForm.size>0) {
          const projeIdNum = form.projeId?parseInt(form.projeId):null;
          siparisler.forEach(sp=>{
            if(projeIdNum && sp.projeId!==projeIdNum) return;
            if(!['taslak','onaylandi','kismen_teslim'].includes(sp.durum)) return;
            if((sp.kalemler||[]).some(k=>k.butceKalemiId && bkIdsInForm.has(k.butceKalemiId))){
              ilgiliSpIds.add(sp.id);
            }
          });
        }
        for(const spId of ilgiliSpIds) {
          const sp = siparisler.find(s=>s.id===spId);
          if(!sp) continue;
          const yeniDurum = computeSpDurumu(sp, tumFaturalar);
          if(yeniDurum && yeniDurum!==sp.durum) {
      ...
      // OTOMATİK MALİYET KALEMİ KAPATMA — per-alt-satır + aggregate fallback
      {
        const bkIds = new Set();
        ...
        for(const bkId of bkIds) {
          ...
          let yeniSat=altlar.map(s=>s.tamamlandi?s:(altSatirEslesti(s, bk.id, tumFaturalar)?{...s,tamamlandi:true}:s));
          let altlarHepsiKapali=yeniSat.every(s=>s.tamamlandi===true);
          let yeniAnaKapali=false;
          if(!altlarHepsiKapali&&isBkAutoClosed(bk, tumFaturalar)){
            yeniSat=yeniSat.map(s=>({...s,tamamlandi:true}));
            altlarHepsiKapali=true;
          }
          if(altlarHepsiKapali) yeniAnaKapali=true;
          const satDegisti=JSON.stringify(yeniSat)!==JSON.stringify(altlar);
          if(satDegisti||yeniAnaKapali){
            const patchData={planlanan_satirlari:yeniSat};
            if(yeniAnaKapali){patchData.tamamlandi=true; patchData.tamamlandi_kaynak='otomatik';}
            try { await sbPatch('butce_kalemleri', bkId, patchData); } catch(e) { console.warn("Maliyet kalemi oto-kapatma hatası:", e.message); }
            setButceKalemleri(prev=>prev.map(b=>b.id===bkId?{...b, planlananSatirlari:yeniSat, ...(yeniAnaKapali?{tamamlandi:true,tamamlandiKaynak:'otomatik'}:{})}:b));
          }
        }
      }
      setFaturalar(prev => {
  ...
  const delSunum = async (id) => {
    if(!confirm("Bu sunum kaydı silinsin mi?")) return;
  ...
  const saveButceKalemi = async (kalem, projeId) => {
    ...
    try {
      ...
      // Satırlar sync (DIFF-BASED — satır ID'leri korunur, sipariş/fatura bağları kopmaz)
      try {
        const dbSatirlar = await sbReq(`butce_kalemi_satirlari?butce_kalemi_id=eq.${savedId}&select=id`);
        const dbIds = new Set((dbSatirlar||[]).map(s=>s.id));
        const stateSatirlar = kalem.planlananSatirlari||[];
        const stateIds = new Set(stateSatirlar.map(s=>s.id));
        // SİL — DB'de var, state'te yok
        const silinecek = [...dbIds].filter(id=>!stateIds.has(id));
        if(silinecek.length>0){
          try { await sbReq(`butce_kalemi_satirlari?id=in.(${silinecek.join(',')})`, {method:'DELETE', headers:{'Prefer':'count=none'}}); }
          catch(e){ console.warn("Satır batch silme hatası:", e.message); }
        }
        // EKLE — state'te var, DB'de yok (yeni satır = Date.now() ID'leri)
        const yeniler = stateSatirlar.filter(s=>!dbIds.has(s.id));
        if(yeniler.length>0){
          try {
            const yeniDbRows = yeniler.map(s=>kalemSatirToDb(s, savedId));
            const savedYeniler = await sbPost('butce_kalemi_satirlari', yeniDbRows);
            yeniler.forEach((s,i)=>{ if(savedYeniler && savedYeniler[i]) s.id = savedYeniler[i].id; });
          } catch(e){ console.warn("Satır batch ekleme hatası:", e.message); }
        }
        // GÜNCELLE — her ikisinde de var (mevcut satırlar)
        const mevcutlar = stateSatirlar.filter(s=>dbIds.has(s.id));
        for(const s of mevcutlar){
          try { await sbPatch('butce_kalemi_satirlari', s.id, kalemSatirToDb(s, savedId)); }
          catch(e){ console.warn(`Satır güncelleme hatası (id=${s.id}):`, e.message); }
        }
        // SATIR REVİZYONLARI sync — her satır için yeni revizyonları INSERT et (silme/güncelleme yok şimdilik)
        for(const s of stateSatirlar){
          if(!s.id || !(s.revizyonlar||[]).length) continue;
          try {
            const dbRevs = await sbReq(`butce_kalemi_satir_revizyonlari?butce_kalemi_satir_id=eq.${s.id}&select=id`);
            const dbRevIds = new Set((dbRevs||[]).map(r=>r.id));
            const yeniRevs = (s.revizyonlar||[]).filter(r=>!dbRevIds.has(r.id));
            if(yeniRevs.length>0){
              const yeniRevRows = yeniRevs.map(r=>satirRevToDb(r, s.id));
              const savedRevs = await sbPost('butce_kalemi_satir_revizyonlari', yeniRevRows);
              yeniRevs.forEach((r,i)=>{ if(savedRevs && savedRevs[i]) r.id = savedRevs[i].id; });
            }
            // Silinen revizyonlar (state'te yok ama DB'de var)
            const stateRevIds = new Set((s.revizyonlar||[]).map(r=>r.id));
            const silinenRevs = [...dbRevIds].filter(id=>!stateRevIds.has(id));
            if(silinenRevs.length>0){
              try { await sbReq(`butce_kalemi_satir_revizyonlari?id=in.(${silinenRevs.join(',')})`, {method:'DELETE', headers:{'Prefer':'count=none'}}); }
              catch(e){ console.warn("Satır revizyon silme hatası:", e.message); }
            }
          } catch(e){ console.warn(`Satır revizyon sync hatası (satirId=${s.id}):`, e.message); }
        }
      } catch(e) { console.warn("Satır sync hatası:", e.message); }
      return savedId;
    } catch(e) {
      ...
      <div style={{height:satici?`${SATICI_UI.headerH}px`:"56px",minHeight:satici?`${SATICI_UI.headerH}px`:"56px",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",background:"#fff",borderBottom:`1px solid ${T.border}`,boxShadow:T.sh}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"12px",marginRight:satici?`${SATICI_UI.rightShift}px`:"0"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:"8px",padding:"4px 10px 4px 4px",borderRadius:"20px",border:`1px solid ${T.border}`,background:"#fafafa"}}>
            ...
            <div style={{fontSize:"11px",color:T.t3,padding:"2px 6px",background:"#fff",borderRadius:"3px",border:`1px solid ${T.border}`}}>{rolLabel(currentUser.rol)}</div>
          </div>
